...
const KERNEL_WRITE_MODE = (typeof process !== 'undefined' ? process.env.VITE_KERNEL_MODE : undefined) || 'PURE' as 'HYBRID' | 'PURE';
        ...
        if (!result.error && result.data && context.tenantId && this.isShadowTable(table)) {
            try {
                ...
                if (entityType) {
                    ...
                    const reconType = entityType as import('./ReconciliationEngine').ReconEntity;
