    ...
    public static async judge(tenantId: string, options?: { commit?: boolean }): Promise<RealityVerdict> {
        ...
        if (import.meta.env.DEV) {
            ...
            if (options?.commit) {
                ...
                await client.from('gm_restaurants')
                    .update({
        ...
        const productCount = productError ? 0 : (Array.isArray(productRows) ? productRows.length : 0);
        ...
        const { data: staffRows, error: staffError } = await client
            ...
            .eq('restaurant_id', tenantId)
            .eq('active', true);
        const staffCount = staffError ? 0 : (Array.isArray(staffRows) ? staffRows.length : 0);
