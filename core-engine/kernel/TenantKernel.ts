...
import { TransitionRequest, TransitionResult } from "../executor/CoreExecutor";
import { ExecutionContext, ExecutionLifecycle } from "./ExecutionContext";
...
export class TenantKernel {
    ...
    constructor(
        context: KernelOptions,
        eventStore: EventStore
    ) {
        if (!context.tenantId) {
            throw new Error("[TenantKernel] FATAL: Cannot birth a Kernel without TenantId.");
        }
   ...
   */
    async boot(): Promise<void> {
        ...
        console.log(`[Kernel:${this.tenantId}] 🟢 Booting (ExecID: ${this.executionId})...`);
        try {
   ...
   */
     ...
     */
    dispose(): void {
        console.log(`[Kernel:${this.tenantId}] 🔴 Disposing (ExecID: ${this.executionId})...`);
    ...
    private assertReady() {
        if (this.lifecycle !== "ACTIVE") {
            throw new Error(`[Kernel:${this.tenantId}] Violation: Cannot execute in lifecycle ${this.lifecycle} (ExecID: ${this.executionId})`);
        }
    }
