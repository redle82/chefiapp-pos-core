...
-- 0. Ensure gm_terminals table exists (if not already created) -----------
...
CREATE TABLE IF NOT EXISTS public.gm_device_install_tokens (
    ...
    device_type TEXT NOT NULL DEFAULT 'APPSTAFF'
                CHECK (device_type IN ('TPV', 'KDS', 'APPSTAFF')),
...
COMMENT ON FUNCTION public.create_device_install_token IS
  ...
  'TTL defaults to 15 minutes (enough time to scan QR and install app). Called from Admin > Dispositivos.';
