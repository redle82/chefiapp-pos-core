...
export function determineBuildModeEnabled(tsconfigFileName) {
    ...
    const useProjectReferences = 'references' in
...
export function retrieveReferencedProjects(parsedConfig, fromDirName) {
    if (Array.isArray(parsedConfig.config?.references)) {
        return parsedConfig.config?.references.map((reference) => path.resolve(fromDirName, ts.resolveProjectReferencePath(reference)));
    }
    return [];
}
/**
