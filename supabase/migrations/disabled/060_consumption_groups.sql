-- Migration: 060_consumption_groups.sql
...
-- Assumindo que gm_order_items já existe
...
DECLARE
    total DECIMAL(10,2);
BEGIN
    SELECT COALESCE(SUM((price_snapshot / 100.0) * qty), 0)
    INTO total
...
SELECT 
    cg.*,
    COALESCE(SUM((oi.price_snapshot / 100.0) * oi.qty), 0) AS total_amount,
...
-- 8. Add new event types to operational_event_type enum
-- (Se o enum já existir, usar ALTER TYPE)
DO $$
BEGIN
    ...
    ELSE
        ...
        ALTER TYPE operational_event_type ADD VALUE IF NOT EXISTS 'consumption_group_created';
        ALTER TYPE operational_event_type ADD VALUE IF NOT EXISTS 'item_assigned_to_group';
        ALTER TYPE operational_event_type ADD VALUE IF NOT EXISTS 'item_moved_between_groups';
        ALTER TYPE operational_event_type ADD VALUE IF NOT EXISTS 'consumption_group_paid';
        ALTER TYPE operational_event_type ADD VALUE IF NOT EXISTS 'partial_table_paid';
    END IF;
EXCEPTION
...
-- 9. Feature Flag (GovernManage)
-- Adicionar feature flag para ativação controlada
...
SELECT 
    ...
    '{"description": "Permite dividir contas em grupos de consumo durante o pedido, não no final", "requires_ui": true}'::jsonb
