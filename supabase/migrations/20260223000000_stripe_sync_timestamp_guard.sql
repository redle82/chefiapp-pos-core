...
-- Ref: docker-core/schema/migrations/20260223_stripe_sync_timestamp_guard.sql
-- =============================================================================

-- 1. Add dedicated billing event timestamp column (defensive)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'gm_restaurants')
     AND NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'gm_restaurants' AND column_name = 'last_billing_event_at')
  THEN
    ALTER TABLE public.gm_restaurants ADD COLUMN last_billing_event_at TIMESTAMPTZ;
  END IF;
END;
$$;

-- 2. Replace function with timestamp-guarded version (defensive checks for Supabase)
...
BEGIN
  ...
  -- 2. Stale event guard
  IF p_event_created_at IS NOT NULL AND v_has_last_event THEN
  ...
  IF p_event_type IN ('customer.subscription.created', 'customer.subscription.updated', 'customer.subscription.deleted') THEN
    ...
    IF v_has_billing_status THEN
      IF v_has_last_event THEN
        ...
        SET billing_status = v_core_status,
      ...
      ELSE
        ...
        WHERE id = v_restaurant_id;
      END IF;
      IF FOUND AND v_has_trial_ends THEN
