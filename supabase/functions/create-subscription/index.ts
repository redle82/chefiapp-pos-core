...
serve(async (req) => {
    ...
    try {
        ...
        // Buscar plano
        const plan = DEFAULT_PLANS.find(p => p.plan_id === plan_id) || DEFAULT_PLANS[1] // Default: Professional
        ...
        if (insertError) {
            console.error('[create-subscription] Error inserting subscription:', insertError)
