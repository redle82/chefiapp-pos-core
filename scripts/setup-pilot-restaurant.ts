...
interface PilotConfig {
  ...
  tables: number;
  menuCategories: Array<{ name: string; items: Array<{ name: string; price_cents: number }> }>;
...
async function createPilotRestaurant(supabase: SupabaseClient, config: PilotConfig): Promise<string> {
  ...
  try {
    const { data: tenantData, error: tenantError } = await supabase
      ...
      .insert({
        ...
        slug: `${config.restaurantName.toLowerCase().replace(/\s+/g, '-')}-${Date.now()}`,
      })
      .select('id')
      .single();
  ...
  const restaurantData: Record<string, unknown> = {
    ...
    slug: `${config.restaurantName.toLowerCase().replace(/\s+/g, '-')}-pilot-${Date.now()}`,
  };
  ...
  if (restaurantError) throw new Error(`Failed to create restaurant: ${restaurantError.message}`);
  
  console.log(`   ✅ Restaurante criado: ${restaurant.name} (${restaurant.id})`);
  return restaurant.id;
}

async function createPilotTables(supabase: SupabaseClient, restaurantId: string, count: number): Promise<string[]> {
...
async function createPilotMenu(supabase: SupabaseClient, restaurantId: string, categories: PilotConfig['menuCategories']): Promise<number> {
  ...
  for (const category of categories) {
    ...
    if (categoryError) {
      console.warn(`   ⚠️  Erro ao criar categoria ${category.name}: ${categoryError.message}`);
      continue;
    }
    ...
    if (productError) {
      console.warn(`   ⚠️  Erro ao criar produtos da categoria ${category.name}: ${productError.message}`);
      continue;
    }
...
async function createPilotStaff(supabase: SupabaseClient, restaurantId: string, staff: PilotConfig['staff']): Promise<number> {
...
async function setupPilotRestaurant(config: PilotConfig = DEFAULT_PILOT_CONFIG): Promise<void> {
...
export { setupPilotRestaurant, PilotConfig };
