...
async function assess() {
  ...
  console.log(
    `Reality Status (DB): ${tenant.reality_status || "NOT SET/DRAFT"}`,
  );
  ...
  for (const [key, path] of Object.entries(ROADMAP_FILES)) {
    ...
    console.log(
      `[${exists ? "WARN" : "MISS"}] ${key}: ${exists ? "PRESENT" : "MISSING"}`,
    );
  ...
  const report = {
    global: {
      ...
      code_coverage_key_features:
        (Object.values(codeStatus).filter((x) => x).length /
          Object.keys(codeStatus).length) *
        100,
    },
