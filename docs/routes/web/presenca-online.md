...
- Config: `/config` (Identidade, Localização — slug, QR); Público: `/public/:slug`. Sidebar "Presença Online" pode abrir `/public/:slug` (preview) ou secção em Config; no código actual route é `/public/trial-restaurant` — em produção usar slug do restaurante.
...
- [ ] Parcial
- [x] Funcional
- Observações: Config (Identidade, PublicQRSection); PublicWebPage (`/public/:slug`). Sidebar "Presença Online" com route `/public/trial-restaurant` — substituir por slug dinâmico em produção.

## 10. Próximos Passos Técnicos

- [ ] Garantir que links "Presença Online" usam slug do restaurante em vez de `trial-restaurant` em produção.
