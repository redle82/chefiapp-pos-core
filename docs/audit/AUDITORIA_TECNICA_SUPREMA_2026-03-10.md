...
O ChefIApp apresenta uma arquitetura madura, fortemente contratualizada e com fronteiras claras entre **Core soberano**, **runtime** e **apps** (`merchant-portal`, `mobile-app`, `customer-portal`). O `merchant-portal` respeita os contratos de navegação e soberania descritos em `ARQUITETURA_GERAL_CHEFIAPP_AS_IS.md` e no canon AppStaff, e o Core mantém-se protegido por `RuntimeReader`/`RuntimeWriter`, `DbWriteGate`, RLS e scripts de auditoria, garantindo que o frontend não escreve diretamente em estado financeiro. A suíte de testes é extensa (Vitest + Playwright + Jest raiz + testes massivos), mas o run de `npm -w merchant-portal run test -- --run` atualmente termina com **7 ficheiros de teste falhados (167 totais, 158 passados, 2 skipped)**, concentrados em **CoreExecutor audit**, **orderLifecycle** e fluxos de **dispositivos/módulos**, o que reduz a confiança operacional imediata. Do ponto de vista de DevOps/DX, o fluxo de build (marketing-only vs completo) está bem documentado, com scripts de auditoria fortes (`audit:*`, `validate:*`), mas exige disciplina e entendimento prévio para ser usado como gate consistente de release.
...
- `customer-portal` existe como workspace com `package.json`, mas também sem fontes visíveis (`src/**` ausente nesta pasta na cópia atual).  
...
- **Merchant-portal (Vitest + Playwright)**  
  ...
  - A execução de `npm -w merchant-portal run test -- --run` neste ambiente resultou em:  
    - **Test Files: 7 failed | 158 passed | 2 skipped (167)**.  
    - Falhas concentradas em:  
      - `tests/core/CoreExecutor.audit.test.ts` — **CoreExecutor Audit Enforcement** (detetar conflito de concorrência otimista).  
      - `src/core/db/index.test.ts` — stub branch `coreNotImplemented`.  
      - `src/core/operational/__tests__/orderLifecycle.test.ts` — múltiplos cenários `processOrderLifecycle` (criação DRAFT, reservas de stock, marcação de indisponibilidade, envio para cozinha, finalização/cancelamento, ciclo completo e múltiplas encomendas).  
      - `src/features/admin/devices/*.verify.test.tsx` e `*InstallQRPanel.antiregression.test.tsx` — fluxo de instalação TPV/KDS desktop-only, QR codes, ausência de instruções iOS/Android e cobertura i18n.  
      - `src/features/admin/modules/pages/ModulesPage.*.test.tsx` — anti-regressão e fluxo desktop (secondary actions de TPV, ligação a `/admin/devices` com filtro de módulo, manifest PWA e ausência de referências a PWA install na fonte).  
  - As mensagens de warning em `PaymentModal.test.tsx`, `SplitBillModalWrapper.test.tsx` e `DataPrivacyPage.test.tsx` são principalmente sobre `act(...)` e pequenos detalhes visuais (uso de `border` vs `borderColor`), não críticas, mas apontam a oportunidades de endurecer testes de UI.
...
**Conclusão testes/auditorias:** A malha de testes e auditorias é profunda e bem distribuída, mas o estado atual **não é “green across the board”**: antes de qualquer release crítica, é necessário recuperar estes 7 ficheiros de teste falhados e garantir que `audit:release:portal` passa consistentemente.
...
| Billing vs operação        | Implementação parcial em `AppContentWithBilling`, docs ainda mencionam gate parcial           | P1         | Confusão entre comportamento real (UI) e contrato; risco de divergência se não alinhado      | `App.tsx` (`AppContentWithBilling`), `BILLING_SUSPENSION_CONTRACT` em `ARQUITETURA_GERAL_CHEFIAPP_AS_IS.md`       |
| Testes portal (Vitest)     | Run completo de `merchant-portal` com 1 ficheiro de teste falhado (`coreNotImplemented` stub) | P1         | Baseline praticamente verde; risco residual limitado ao teste de stub `coreNotImplemented` agora alinhado com Logger, mas convém manter `--run` como rotina antes de refactors grandes | Último run de `npm -w merchant-portal run test -- --run` (1 failed, 164 passed, 2 skipped) antes do ajuste em `src/core/db/index.test.ts` |
