...
## PR Cockpit — "Do I need E2E?" (5-second decision)

> **One question:** can this PR break _how the app boots_ or _what the deploy serves_?
>
> **No** → merge when CI is green. **Yes** → add label `run-e2e`.

### SKIP E2E — safe to merge with green CI only

**Surface changes** — UI components, styling, copy text, tests, mocks, fixtures, docs, README.

### ADD `run-e2e` — any of these 4 categories

| Category          | Examples                                                                         |
| ----------------- | -------------------------------------------------------------------------------- |
| **Boot path**     | `App.tsx`, `*Routes.tsx`, `FlowGate`, `useAuth`, `useCoreAuth`, `TenantResolver` |
| **Build / infra** | `vite.config.ts`, `tsconfig`, `vercel.json`, PWA / SW / manifest, CI workflows   |
| **Core gates**    | `sovereignty-gate.sh`, `check-financial-supabase.sh`, `contract-gate.sh`         |
| **Dependencies**  | `package.json`, `pnpm-lock.yaml` (any change, even a patch bump)                |

**Mnemonic: B-B-C-D** _(Boot, Build, Core gates, Deps)_ → label it.
...
| --------- | --------------------------------------- | ---------------------------------------- |
| `[CI]`    | Workflow, lint, typecheck, gates        | `[CI] add artifact validation step`      |
| `[PWA]`   | Service worker, manifest, workbox       | `[PWA] switch REST cache to NetworkOnly` |
| `[UI]`    | Components, pages, styling              | `[UI] redesign order card layout`        |
| `[Core]`  | FlowGate, routing, TenantResolver, auth | `[Core] fix TenantRole→UserRole mapping` |
| `[Build]` | Vite config, Vercel, deploy pipeline    | `[Build] add smoke HTTP 200 to CI`       |
| `[Feat]`  | New user-facing feature                 | `[Feat] add table QR code generation`    |
| `[Fix]`   | Bug fix                                 | `[Fix] resolve 401 ghost from SW cache`  |
| `[Docs]`  | Documentation only                      | `[Docs] update CI_GOVERNANCE`            |
| `[Chore]` | Maintenance, cleanup, deps              | `[Chore] upgrade framer-motion to 12.x`  |

**Labels to know:**

| Label      | Effect                                           |
| ---------- | ------------------------------------------------ |
| `run-e2e`  | Triggers non-blocking E2E Playwright suite in CI |
| `breaking` | Signals API/contract change — review carefully   |
| `hotfix`   | Fast-track: skip non-essential reviewers         |

---

_Maintained by the ChefIApp engineering team. Update this file when CI policy changes._
