...
| [RUNTIME_CONNECTIVITY_CONTRACT.md](./RUNTIME_CONNECTIVITY_CONTRACT.md)       | **(CANONICAL / HIGH-RANK)** Runtime never discovers Core. Explicit config only via env vars. Missing config = fatal error in prod. Port 3001 is indicator only.                                                                                                                                                 | Connectivity fixes, client setup, Docker/Supabase mode determination.                              |
...
| [BILLING_OPERATIONAL_CONTRACT.md](./BILLING_OPERATIONAL_CONTRACT.md)         | **Billing as Perception.** Define estados trial, active, past_due, suspended. DS vira sensor de billing antes do gate técnico.                                                                                                                                                                                  | Implementação de faturação, avisos de pagamento, bloqueio de operação por falta de pagamento.      |
...
| [SESSION_RESUME_CONTRACT.md](./SESSION_RESUME_CONTRACT.md)                                     | **Retoma de sessão (last-context).** "Já tenho acesso" → após login, redireccionar para última rota válida (dashboard, /op/tpv, /op/kds, /op/cash) ou /app/dashboard. Estende AUTH_AND_ENTRY.                                                                               | Implementação "Já tenho acesso"; persistência e leitura de último contexto.                                  |
| [DEMO_MODE_CONTRACT.md](./DEMO_MODE_CONTRACT.md)                                                 | **Modo Demo.** Observação sem identidade forte; /demo; nunca cria restaurante, nunca activa piloto, nunca escreve no Core; isDemo em GlobalUIState.                                                                                                                        | Página /demo; CTAs "Explorar o Sistema (Demo)"; isolamento Demo do Core.                                   |
...
| [CANONICAL_ROUTES_BY_MODE.md](./CANONICAL_ROUTES_BY_MODE.md)                                     | **Mapa canónico de rotas por modo (Demo / Piloto / Operacional).** Rota → modo; três portais da landing; diagrama por modo.                                                                                                                                                 | Saber que rotas pertencem a que modo; desenho de fluxos por modo; auditoria landing → modo.                |
| [NAVIGATION_OPERATIONAL_CONTRACT.md](./NAVIGATION_OPERATIONAL_CONTRACT.md)                       | **Mapa de estados do utilizador (5 estados).** Regras de transição; visitante nunca /op/* sem mode=demo; TPV demo nunca volta à landing; landing nunca fallback de erro.                                                                                                     | Navegação operacional; hierarquia Demo → TPV demo → /demo; eliminar teletransporte cognitivo.             |
...
| [LANDING_STATE_ROUTING_CONTRACT.md](./LANDING_STATE_ROUTING_CONTRACT.md)         | **Landing como launcher do OS.** Botões como portais de estado (não só páginas); modos Demo / Piloto / Operacional; três caminhos: Operar / Explorar demo / Já tenho acesso; intenção + contratos por portal. Complementa CORE_LANDING_ROUTES com semântica estado. | CTAs da landing, modos do sistema, copy "Explorar o Sistema (Demo)", resumo de sessão, decisões de produto na entrada.                      |
