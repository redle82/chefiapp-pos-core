...
> ⚠️ **PARTIALLY SUPERSEDED** — Referências a `AppDomainWrapper` foram eliminadas (PR-G, refactor/boot-pipeline). A árvore de providers vive em `App.tsx → TenantProvider`. Ler com esta nota em mente.
...
| **TESTS**       | E2E (Playwright), unit (Vitest).                                                                                                         | merchant-portal/tests/e2e/_.spec.ts, merchant-portal/src/\*\*/_.test.ts(x), merchant-portal/tests/core/\*.test.ts.                                           |
...
- **Onde:** Montado em App.tsx: `<Route element={<RoleGate />}>` envolve todas as rotas de gestão e operação (dashboard, config, /app/_, /op/_, etc.). Não envolve `/bootstrap`, `/app/select-tenant` nem rotas públicas.
...
- **OPERATIONAL (/op):** Rotas /op/tpv e /op/kds fechadas; RequireOperational estável (isPublished); TPV/KDS com wrapper fullscreen; contratos OPERATIONAL\_\* e CORE_TPV/KDS alinhados.
