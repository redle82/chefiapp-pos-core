...
> ⚠️ **PARTIALLY SUPERSEDED** — References to `AppDomainWrapper` in the boot chain are stale. That component was deleted in PR-G (refactor/boot-pipeline). The canonical boot chain is `FlowGate → TenantProvider → RequireActivation`.
...
**Scope:** Specific business tools running _inside_ the Kernel.
