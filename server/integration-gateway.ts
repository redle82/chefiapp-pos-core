...
const CORE_SERVICE_KEY = process.env.CORE_SERVICE_KEY || process.env.CORE_ANON_KEY || "";
const INTERNAL_API_TOKEN = process.env.INTERNAL_API_TOKEN || "chefiapp-internal-token-dev";
...
function apiError(error: string, message: string, details?: Record<string, unknown>): object {
...
async function fetchWebhookConfigs(restaurantId: string): Promise<
  Array<{ id: string; url: string; secret: string; events: string[]; enabled: boolean }>
> {
  ...
  return Array.isArray(rows)
    ? rows.map((r: { id: string; url: string; secret: string; events: unknown; enabled: boolean }) => ({
        id: r.id,
        url: r.url,
        secret: r.secret,
...
async function insertDeliveryLog(entry: {
  ...
  if (!res.ok) {
    console.error("[integration-gateway] Failed to insert delivery log:", res.status, await res.text());
  }
}
...
// API Key auth (API v1)
// ---------------------------------------------------------------------------
async function lookupApiKey(keyHash: string): Promise<{ id: string; restaurant_id: string } | null> {
  if (!CORE_SERVICE_KEY) return null;
  const res = await fetch(
    `${REST}/api_keys?key_hash=eq.${encodeURIComponent(keyHash)}&select=id,restaurant_id`,
    { headers: coreHeaders() }
  );
...
async function handleInternalEvents(body: string): Promise<{ status: number; json: object }> {
  let parsed: InternalEventBody;
  try {
    parsed = JSON.parse(body) as InternalEventBody;
  } catch {
    return { status: 400, json: { error: "invalid_json", message: "Invalid JSON body" } };
  }
  const { event, restaurant_id, payload } = parsed;
  if (!event || typeof event !== "string" || !restaurant_id || typeof restaurant_id !== "string") {
    return { status: 400, json: { error: "bad_request", message: "event and restaurant_id required" } };
  }
  ...
  const webhookPayload = buildWebhookPayload(deliveryId, event, restaurant_id, payload ?? {});

  await Promise.all(sendAll.map((c) => deliverOne(c, restaurant_id, webhookPayload)));

  return { status: 202, json: { accepted: true, delivery_id: deliveryId, endpoints: sendAll.length } };
}

// ---------------------------------------------------------------------------
// API v1 handlers
// ---------------------------------------------------------------------------
async function emitEventInternal(event: string, restaurantId: string, payload: Record<string, unknown>): Promise<void> {
  const configs = await fetchWebhookConfigs(restaurantId);
  const sendAll = configs.filter((c) => c.events.length === 0 || c.events.includes(event));
  const deliveryId = `wh_evt_${crypto.randomUUID()}`;
  const webhookPayload = buildWebhookPayload(deliveryId, event, restaurantId, payload);
  await Promise.all(sendAll.map((c) => deliverOne(c, restaurantId, webhookPayload)));
}
...
async function handleApiV1(
  ...
  if (path === "/api/v1/orders" && method === "POST") {
    let body: { items?: Array<{ product_id: string; quantity: number; unit_price: number }>; table_id?: string; source?: string };
    try {
      body = JSON.parse(bodyStr || "{}") as typeof body;
    } catch {
      return { status: 400, json: apiError("validation_error", "Invalid JSON", "Body must be JSON") };
    }
    const items = body?.items;
    if (!Array.isArray(items) || items.length === 0) {
      return { status: 400, json: apiError("validation_error", "items required", "items must be a non-empty array") };
    }
    const pItems = items.map((it: { product_id?: string; quantity?: number; unit_price?: number }) => ({
      product_id: it.product_id,
      quantity: it.quantity ?? 1,
      unit_price: it.unit_price ?? 0,
    }));
    const syncMetadata: Record<string, unknown> = { origin: body?.source ?? "api" };
    ...
    try {
      ...
      if (!res.ok) {
        ...
        return { status: 400, json: apiError("validation_error", "Order creation failed", t) };
      }
      ...
      await emitEventInternal("order.created", restaurantId, {
        ...
        source: "online",
        items: pItems.map((it: { product_id: string; quantity: number; unit_price: number }) => ({
          id: it.product_id,
          name: "",
          quantity: it.quantity,
          priceCents: it.unit_price,
        })),
        totalCents: pItems.reduce((s: number, it: { quantity: number; unit_price: number }) => s + it.quantity * it.unit_price, 0),
        createdAt: Date.now(),
      });
      return { status: 201, json: { orderId: String(orderId), status: "new" } };
    } catch (e) {
      console.error("[integration-gateway] create order", e);
      return { status: 500, json: apiError("internal_error", "Internal server error", String(e)) };
    }
  }
  ...
  if (patchOrderMatch && method === "PATCH") {
    ...
    if (!coreStatus) {
      return { status: 400, json: apiError("validation_error", "status required", "status must be one of: new, preparing, ready, served, paid, cancelled") };
    }
    try {
      const res = await fetch(
        `${REST}/gm_orders?id=eq.${encodeURIComponent(orderId)}&restaurant_id=eq.${encodeURIComponent(restaurantId)}`,
        {
          method: "PATCH",
          headers: coreHeaders(),
      ...
      if (!res.ok) return { status: 404, json: apiError("not_found", "Order not found", { orderId }) };
      await emitEventInternal("order.updated", restaurantId, { orderId, status, updatedAt: Date.now() });
  ...
  if (paymentOrderMatch && method === "POST") {
    ...
    let body: { amountCents?: number; paymentMethod?: string; externalId?: string };
    try {
      body = JSON.parse(bodyStr || "{}") as typeof body;
    } catch {
      return { status: 400, json: apiError("validation_error", "Invalid JSON", "Body must be JSON") };
  ...
  if (path === "/api/v1/integrations/whatsapp/incoming" && method === "POST") {
    ...
    let bodyObj: { entry?: Array<{ changes?: Array<{ value?: { messages?: Array<{ from: string; text?: { body: string }; type: string }> }> }> } };
    try {
      bodyObj = JSON.parse(bodyStr || "{}") as typeof bodyObj;
    } catch {
      return { status: 400, json: apiError("validation_error", "Invalid JSON body") };
    }
    if (appSecret) {
      const raw = reqHeaders?.["x-hub-signature-256"];
      const sig = typeof raw === "string" ? raw : Array.isArray(raw) ? raw[0] : undefined;
      if (!sig || !sig.startsWith("sha256=")) {
        return { status: 401, json: apiError("unauthorized", "Missing or invalid X-Hub-Signature-256") };
      }
      const expected = "sha256=" + hmacSha256Hex(appSecret, bodyStr);
      if (sig !== expected) {
        return { status: 401, json: apiError("unauthorized", "Invalid webhook signature") };
      }
    }
    ...
    if (messages?.length) {
      ...
      const payload = {
        ...
        items: [{ id: "wa-1", name: text.slice(0, 200) || "Pedido WhatsApp", quantity: 1, priceCents: 0 }],
        totalCents: 0,
        customerPhone: from,
        createdAt: Date.now(),
      };
  ...
  if (path === "/api/v1/tasks" && method === "POST") {
    let body: { title?: string; description?: string; priority?: string; assigneeRole?: string };
    try {
      body = JSON.parse(bodyStr || "{}") as typeof body;
    } catch {
      return { status: 400, json: apiError("validation_error", "Invalid JSON", "Body must be JSON") };
    ...
    if (!body?.title || typeof body.title !== "string") {
      return { status: 400, json: apiError("validation_error", "title required", "title must be a non-empty string") };
    }
    ...
    try {
      ...
      if (!res.ok) {
        ...
        return { status: 500, json: apiError("internal_error", "Failed to create task", await res.text()) };
      }
  ...
  try {
    ...
    if (path === "/internal/events" && method === "POST") {
      ...
      if (token !== INTERNAL_API_TOKEN) {
        ...
        res.end(JSON.stringify({ error: "unauthorized", message: "Invalid or missing internal token" }));
        return;
      }
    ...
    if (path.startsWith("/api/v1/")) {
      ...
      if (!apiKey) {
        res.writeHead(401, { "Content-Type": "application/json" });
        res.end(JSON.stringify(apiError("unauthorized", "API key missing", "Use X-API-Key or Authorization: Bearer <key>")));
        return;
      }
      ...
      if (!keyRow) {
        res.writeHead(401, { "Content-Type": "application/json" });
        res.end(JSON.stringify(apiError("unauthorized", "Invalid API key", "API key not found or revoked")));
        return;
      }
      ...
      if (!rate.ok) {
        ...
        res.end(JSON.stringify(apiError("rate_limit_exceeded", "Too many requests", "Limit 100/min", { retryAfter: rate.retryAfter })));
        return;
      }
      const bodyStr = await parseBody(req);
      const headers: Record<string, string | string[] | undefined> = {};
      if (req.headers) {
        for (const [k, v] of Object.entries(req.headers)) headers[k.toLowerCase()] = v as string | string[];
      }
      const { status: apiStatus, json: apiJson, headers: apiHeaders } = await handleApiV1(method, path, bodyStr, {
        keyId: keyRow.id,
        restaurantId: keyRow.restaurant_id,
      }, headers);
      const h: Record<string, string> = { "Content-Type": "application/json", ...apiHeaders };
...
server.listen(PORT, () => {
  console.log(`[integration-gateway] Listening on http://localhost:${PORT} (health=/health, events=POST /internal/events)`);
});
