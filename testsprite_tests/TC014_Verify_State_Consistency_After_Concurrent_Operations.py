...
async def run_test():
    ...
    try:
        ...
        frame = context.pages[-1]
        # Click 'Entrar (Dev Mode)' to login
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Concurrent Operation Success').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test case failed: Concurrent session or order operations did not maintain consistent state, race conditions or event ordering issues detected as per the test plan.")
