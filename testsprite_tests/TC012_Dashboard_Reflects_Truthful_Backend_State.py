...
async def run_test():
    ...
    try:
        ...
        frame = context.pages[-1]
        # Click on 'Entrar (Dev Mode)' button to login
        elem = frame.locator('xpath=html/body/div/div/div[2]/div/div/div[3]/div/button').nth(0)
        await page.wait_for_timeout(3000); await elem.click(timeout=5000)
        

        # -> Input entity name in the Google or Google Maps link input and proceed.
        frame = context.pages[-1]
        # Input entity name or Google Maps link for onboarding identity
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Backend Confirmation Pending').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test failed: The home dashboard UI advanced state without explicit backend confirmation, violating the test plan requirement of no optimistic updates.")
