...
async def run_test():
    ...
    try:
        ...
        # -> Click 'Não encontrei no Google' to bypass entity name input and continue onboarding.
        frame = context.pages[-1]
        # Click 'Não encontrei no Google' to bypass entity name input.
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Payment Successful with Idempotency Enforcement').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test failed: Payment processing with cash, card, and pix methods did not enforce idempotency as required by the test plan.")
