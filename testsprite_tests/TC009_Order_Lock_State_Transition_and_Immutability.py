...
async def run_test():
    ...
    try:
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Order successfully unlocked and editable').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test case failed: Locking the order did not transition its state to LOCKED or allowed modifications after locking, violating the test plan requirements.")
