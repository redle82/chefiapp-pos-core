...
async def run_test():
    ...
    try:
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Duplicate event detected: Event ID already exists').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError("Test case failed: The event store did not reject the duplicate event as expected, indicating idempotency enforcement failure.")
