...
async def run_test():
    ...
    try:
        ...
        # -> Invoke session start operation via CoreExecutor for an INACTIVE session by logging in with provided credentials.
        ...
        # -> Input email and password, then click login button to invoke session start operation.
        frame = context.pages[-1]
        ...
        frame = context.pages[-1]
        # Click login button to invoke session start operation
        ...
        try:
            ...
            raise AssertionError("Test case failed: The session did not transition from INACTIVE to ACTIVE and the session start event was not recorded atomically in the event store as expected.")
