    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (items: Array<{ quantity: number; price_cents: number }>) => {
          ...
          const lockedEvent = orderEventsStream.find(
            (e: { type: string; payload?: { total_cents?: number } }) =>
              e.type === "ORDER_LOCKED"
          );
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (amountCents: number) => {
          ...
          const confirmedPayment = payments.find((p: Payment) => p.id === payment.id);
          ...
          const confirmedEvent = paymentEventsStream.find(
            (e: { type: string; payload?: { payment_id?: string; state?: string } }) =>
