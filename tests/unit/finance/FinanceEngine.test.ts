...
const mockOrders = [
  { id: "o1", total_cents: 2500, payment_method: "credit", created_at: "2026-02-09T12:30:00Z", status: "completed" },
  { id: "o2", total_cents: 1500, payment_method: "cash", created_at: "2026-02-09T13:15:00Z", status: "completed" },
  { id: "o3", total_cents: 3000, payment_method: "credit", created_at: "2026-02-09T20:00:00Z", status: "paid" },
];
          ...
          eq: vi.fn().mockReturnValue({
            gte: vi.fn().mockReturnValue({
              lte: vi.fn().mockReturnValue({
                ...
                not: vi.fn().mockResolvedValue({ data: mockOrders, error: null }),
              }),
            }),
          }),
        }),
    ...
    if (table === "gm_z_reports") {
      return {
        insert: vi.fn().mockReturnValue({
          select: vi.fn().mockReturnValue({
            single: vi.fn().mockResolvedValue({ data: { id: "z-2026-02-09" }, error: null }),
          }),
        }),
        select: vi.fn().mockReturnValue({
          eq: vi.fn().mockReturnValue({
            single: vi.fn().mockResolvedValue({
              data: { id: "z-2026-02-09", total_gross: 7000, total_net: 4900, cash_diff: 50 },
              error: null,
            }),
          }),
        }),
...
describe("FinanceEngine (Core-backed)", () => {
  it("should aggregate daily revenue from gm_orders", async () => {
    ...
    vi.doMock("../../../merchant-portal/src/core/infra/dockerCoreFetchClient", () => ({
      getDockerCoreFetchClient: () => ({ from: mockFrom, rpc: mockRpc }),
    }));
    const mod = await import("../../../merchant-portal/src/core/reports/FinanceEngine");
  ...
  it("should calculate hourly sales distribution", async () => {
    ...
    const hourlyTotal = Object.values(snapshot.hourlySales as Record<string, number>).reduce((s: number, v: number) => s + v, 0);
  ...
  it("should close day with Z-report", async () => {
    ...
    const result = await mod.FinanceEngine.closeDay("rest-1", 1550, "Caixa fechado OK");
  ...
  it("should return empty snapshot when Core is down", async () => {
    vi.resetModules();
    vi.doMock("../../../merchant-portal/src/core/infra/dockerCoreFetchClient", () => ({
      getDockerCoreFetchClient: () => { throw new Error("offline"); },
    }));
    const mod = await import("../../../merchant-portal/src/core/reports/FinanceEngine");
