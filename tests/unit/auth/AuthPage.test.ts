...
// Mock Supabase
const mockSignInWithOAuth = jest.fn() as jest.MockedFunction<
  (options: any) => Promise<any>
>;
const mockSignInWithPassword = jest.fn() as jest.MockedFunction<
  (credentials: any) => Promise<any>
>;
const mockSignUp = jest.fn() as jest.MockedFunction<
  (credentials: any) => Promise<any>
>;
...
describe("AuthPage - Autenticação Pura", () => {
  ...
  describe("OAuth Google", () => {
    ...
    it("deve usar redirectTo correto baseado em window.location.origin", () => {
      ...
      expect(redirectUrl).toBe("http://localhost:5175/app");
    });
  });
