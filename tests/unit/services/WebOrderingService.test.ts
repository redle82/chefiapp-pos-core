...
jest.mock("../../../merchant-portal/src/core/infra/CoreOrdersApi", () => ({
  createOrderAtomic: jest.fn(),
}));

import { createOrderAtomic } from "../../../merchant-portal/src/core/infra/CoreOrdersApi";
import {
...
const mockRpc = (supabase as any).rpc as jest.MockedFunction<
  (
    name: string,
    args: unknown,
  ) => Promise<{ data: { id: string } | null; error: unknown }>
>;
const mockCheckOrderProtection = checkOrderProtection as jest.MockedFunction<
  typeof checkOrderProtection
>;
const mockRecordOrderSubmission = recordOrderSubmission as jest.MockedFunction<
  typeof recordOrderSubmission
>;
const mockCreateOrderAtomic = createOrderAtomic as jest.MockedFunction<
  typeof createOrderAtomic
>;
...
describe("WebOrderingService", () => {
  ...
  beforeEach(() => {
    ...
    mockCreateOrderAtomic.mockResolvedValue({
      data: { id: "order-123", total_cents: 5000, status: "PENDING" },
      error: null,
    });
  });
