...
import {
  getTabIsolated,
  setTabIsolated,
} from "../../../merchant-portal/src/core/storage/TabIsolatedStorage";

const mockGetTabIsolated = getTabIsolated as jest.MockedFunction<
  typeof getTabIsolated
>;
const mockSetTabIsolated = setTabIsolated as jest.MockedFunction<
  typeof setTabIsolated
>;
...
describe("OrderProtection", () => {
  ...
  describe("checkOrderProtection", () => {
    ...
    it("deve aplicar rate limiting", () => {
      ...
      mockGetTabIsolated.mockImplementation((key: string) => {
        if (key === "chefiapp_order_ratelimit_rest-123") {
          return JSON.stringify(rateLimitRecord);
        }
        return null;
      });
