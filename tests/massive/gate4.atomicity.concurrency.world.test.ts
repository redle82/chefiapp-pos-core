...
import { CoreEvent, EventType } from '../../event-log/types';
...
describe('Gate 4: Atomicity & Concurrency (World Simulation)', () => {
    ...
    describe('Concurrent Stream Version Conflicts', () => {
        it('should detect and reject concurrent writes to same stream_version', async () => {
            ...
            const attempts: { streamId: string; version: number; succeeded: boolean }[] = [];

            // Pick some orders and try to write concurrently
            const orderEvents = world.allEvents.filter(e => e.stream_id.startsWith('ORDER:'));
            const streams = [...new Set(orderEvents.map(e => e.stream_id))].slice(0, 10);
            ...
            for (const streamId of streams) {
                ...
                const initialEvent: CoreEvent = {
                    ...
                    stream_version: 0,
                    type: 'ORDER_CREATED',
                    payload: { id: streamId.split(':')[1], table_id: `T${Math.floor(rng.next() * 10) + 1}` },
                ...
                const writePromises = Array.from({ length: 5 }, async () => {
                    const event: CoreEvent = {
                        ...
                        payload: { order_id: streamId.split(':')[1], item: { id: rng.uuid() } },
                        occurred_at: new Date(),
                        hash: rng.uuid(),
                    };
                    ...
                    try {
                        ...
                        if (error.message.includes('Concurrency') || error.message.includes('version')) {
                            return false;
                        }
                        throw error;
                    }
        ...
        it('should maintain stream version integrity under concurrent load', async () => {
            ...
            for (let i = 0; i < eventCount; i++) {
                const event: CoreEvent = {
                    ...
                    stream_version: i,
                    type: 'ORDER_ITEM_ADDED',
                    payload: { order_id: streamId.split(':')[1], item: { id: `item_${i}` } },
                    occurred_at: new Date(),
                    hash: rng.uuid(),
                };
    ...
    describe('Webhook Duplication Handling', () => {
        it('should handle duplicate webhook payloads idempotently', async () => {
            ...
            const paymentEvents = world.allEvents.filter(e => e.type === 'PAYMENT_CONFIRMED');

            // Generate duplicates
            const webhookPayloads = stressRunner.generateDuplicateWebhooks(paymentEvents);

            const processedPayments = new Map<string, number>();
            const operations: { idempotencyKey: string; result: string; count: number }[] = [];

            // Process webhooks
            for (const { payload, isDuplicate, delayMs } of webhookPayloads) {
                if (delayMs > 0) {
                    await new Promise(resolve => setTimeout(resolve, Math.min(delayMs, 100)));
                }
            ...
            const uniquePayments = new Set(paymentEvents.map(e => e.payload?.payment_id));
    ...
    describe('Double-Click Pay Prevention', () => {
        it('should prevent duplicate payment creation on rapid clicks', async () => {
            ...
            for (let i = 0; i < paymentAttempts; i++) {
                paymentCreationAttempts.push((async () => {
    ...
    describe('Atomic Rollback Simulation', () => {
        it('should not leave partial state on failure', async () => {
            const operations: { name: string; eventWritten: boolean; sealCreated: boolean; committed: boolean }[] = [];
...
function validateWebhookPayload(payload: any): boolean {
    ...
    if (payload.amountCents !== undefined && payload.amountCents < 0) return false;
    if (payload.status && !['PAID', 'PENDING', 'FAILED'].includes(payload.status)) return false;
    return true;
}
