...
const PAGES_TO_AUDIT = [
  ...
  { path: "/app/publish", name: "PublishPage", needsSetup: true },
  {
    path: "/admin/config/integrations/payments",
    name: "PaymentsPage",
    needsSetup: true,
  },
...
test.describe("Truth Lock — Microcopy Audit", () => {
  ...
  for (const pageInfo of PAGES_TO_AUDIT) {
    test(`${pageInfo.name} has no forbidden time promises`, async ({
      page,
      baseURL,
    }) => {
        ...
        if (match) {
          ...
          if (pattern.source === "\\d+%" && match[0]) {
            ...
            const styleContext = await page.evaluate((text) => {
              ...
              return (
                el.innerHTML.includes(`style="`) && el.innerHTML.includes(text)
              );
            }, match[0]);
          ...
          test.fail(
            true,
            `Found forbidden pattern "${match[0]}" on ${pageInfo.name}`,
          );
        }
      }
    });
  ...
  test("no fake progress bars exist anywhere", async ({ page, baseURL }) => {
    ...
    const progressBars = await page
      .locator('[role="progressbar"], progress, .progress-bar')
      .count();
