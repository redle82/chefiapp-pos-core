...
import { Order, OrderItem, Payment, Session } from '../../core-engine/repo/types';
...
export class WorldFactory {
        ...
        for (const restaurant of restaurants) {
            ...
            for (let s = 0; s < sessionCount; s++) {
                ...
                for (const order of session.orders) {
                    ...
                    const orderTotal = order.items.reduce((sum, item) => sum + item.subtotal_cents, 0);
    ...
    private generateSession(restaurant: Restaurant): GeneratedSession {
        ...
        for (let o = 0; o < orderCount; o++) {
            if (orders.length >= this.config.ordersPerRestaurant / Math.ceil(this.config.ordersPerRestaurant / 10)) {
    ...
    private generateOrder(sessionId: string, table: Table, restaurant: Restaurant): GeneratedOrder {
        ...
        // ORDER_PAID
        const orderPaidEvent = this.createEvent(
            `ORDER:${orderId}`,
            'ORDER_PAID',
            {
                order_id: orderId,
    ...
    private createEvent(streamId: StreamId, type: EventType, payload: Record<string, any>): CoreEvent {
        this.eventSequence++;
        this.advanceTime();
