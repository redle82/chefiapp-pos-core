...
describe("TASK-2.2.3: Fiscal Retry Automatic", () => {
  ...
  beforeAll(() => {
    if (!pool) {
      console.warn(
        "⚠️  DATABASE_URL not set. Skipping fiscal-retry integration tests.",
      );
      return;
    }
  });
  ...
  beforeAll(async () => {
    if (!pool) return;
    const { rows } = await pool.query(
      "SELECT to_regclass('public.gm_fiscal_queue') AS table_name",
    );
    hasFiscalQueueTable = Boolean(rows[0]?.table_name);
    if (!hasFiscalQueueTable) {
      console.warn(
        "⚠️  gm_fiscal_queue table not found. Skipping fiscal-retry tests.",
      );
    }
  });

  beforeEach(async () => {
    if (!pool || !hasFiscalQueueTable) return;
  ...
  it("should mark item as RETRYING when mark_fiscal_queue_failed is called", async () => {
    ...
    if (!hasFiscalQueueTable) {
      console.log("⏭️  Skipping test - gm_fiscal_queue missing");
      return;
    }
    ...
    const orderData = {
      ...
      total_cents: 1000,
      items: [
        { id: "item-1", name: "Test", quantity: 1, price_snapshot: 1000 },
      ],
    };
    const paymentData = {
      method: "cash",
