...
import { setProductMode as setCoreProductMode } from "./core-client";
      ...
      <body style="font-family: monospace; padding: 20px;">
        ...
        <pre>${
          eventLog
            ...
            .map(
              (e) =>
                `${e.time} | ${e.type.padEnd(40)} | ${e.status} ${
                  e.details || ""
                }`,
            )
            .join("\n") || "No events yet"
    ...
    req.on("end", async () => {
      ...
      try {
        ...
        if (result.subscription_update) {
          log(
            `   └─ Subscription: ${result.subscription_update.stripe_subscription_id}`,
          );
          ...
          // Billing → product_mode: assinatura ACTIVE ativa modo live no Core (contrato BILLING_PRODUCT_MODE_CONTRACT)
          if (
            result.subscription_update.status === "ACTIVE" &&
            result.subscription_update.merchant_id
          ) {
            const restaurantId = result.subscription_update.merchant_id;
            const coreResult = await setCoreProductMode(restaurantId, "live");
            if (coreResult.error) {
              log(
                `   └─ ⚠️ Core product_mode update failed: ${coreResult.error}`,
              );
            } else {
              log(
                `   └─ Core product_mode → live (restaurant ${restaurantId})`,
              );
            }
          }
        }
        ...
        if (result.payment_event) {
          log(
            `   └─ Payment: ${result.payment_event.type} (${result.payment_event.amount_cents} cents)`,
          );
        }
