...
export const ERROR_MESSAGES: Record<string, ErrorMessage> = {
  ...
  SLUG_TAKEN: {
    ...
    message:
      "Esse nome já está a ser usado por outro restaurante. Tenta outro.",
...
export function getErrorMessage(error: unknown): ErrorMessage {
  ...
  if (error && typeof error === "object") {
    ...
    const code = errorObject.error || errorObject.code || errorObject.message;
...
export function isRetryableError(error: unknown): boolean {
  ...
  return typeof code === "string" && retryable.includes(code);
}
...
export function isUserFixableError(error: unknown): boolean {
  ...
  return typeof code === "string" && fixable.includes(code);
}
