...
async function applyMigration() {
    ...
    try {
        ...
        const migrationPath = path.resolve(__dirname, '../src/core/supabase/migrations/20260122_inventory_trigger.sql');
        if (!fs.existsSync(migrationPath)) {
            throw new Error(`Migration not found at ${migrationPath}`);
        }

        const sql = fs.readFileSync(migrationPath, 'utf-8');
        console.log(`📜 Applying migration 20260122_inventory_trigger.sql...`);
