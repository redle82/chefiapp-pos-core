...
export const ROLE_PERMISSIONS: Record<TenantRole, string[]> = {
  ...
  staff: ["tpv:view", "tpv:operate", "orders:view", "orders:create"],
  waiter: [
    "tpv:view",
    "tpv:operate",
    "orders:view",
  ...
  kitchen: ["kds:view", "kds:manage", "orders:view"],
};
...
export async function fetchUserMemberships(
  ...
  // members can be an object or array depending on the client (PostgREST single vs many)
  const membersArray = Array.isArray(members) ? members : [members];

  // Fetch restaurant names
  const restaurantIds = membersArray.map((m: any) => m.restaurant_id);
  ...
  return membersArray.map((m: { restaurant_id: string; role: string }) => ({
    restaurant_id: m.restaurant_id,
    restaurant_name:
      restaurantsArray.find(
...
export async function resolve(
  ...
  if (cachedTenantId && cachedStatus === "ACTIVE") {
    const membership = memberships.find(
      (m) => m.restaurant_id === cachedTenantId,
    );
 ...
 */
export function isTenantSealedFor(
...
export function isTenantOwner(
  ...
  return memberships.some(
    (m) => m.restaurant_id === tenantId && m.role === "owner",
  );
}
