...
const MiniSparkline = ({ data, color = colors.action.base }: { data: Record<number, number>; color?: string }) => {
  const hours = Object.keys(data).map(Number).sort((a, b) => a - b);
  if (hours.length === 0) return <div style={{ height: 40, opacity: 0.3 }}>Sem dados</div>;
...
const TrendBadge = ({ value, suffix = '%' }: { value: number; suffix?: string }) => {
  ...
  return (
    <span style={{
      ...
      fontSize: 11,
      fontWeight: 600,
      background: isNeutral ? colors.surface.layer2 : isPositive ? '#10B98120' : '#EF444420',
      color: isNeutral ? colors.text.secondary : isPositive ? '#10B981' : '#EF4444',
    }}>
      {isPositive ? '↑' : isNeutral ? '–' : '↓'} {Math.abs(value)}{suffix}
    </span>
  );
};
...
const MetricCard = ({ 
  ...
  <div style={{
    ...
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
      <Text size="xs" color="secondary">{icon} {label}</Text>
      {trend !== undefined && <TrendBadge value={trend} />}
    </div>
    <Text size="xl" weight="bold" color="primary">{value}</Text>
  ...
  if (error) {
    return (
      <Card surface="layer1" padding="md">
        <Text color="secondary">Erro ao carregar métricas: {error}</Text>
      </Card>
    );
  }
    ...
    <Card surface="layer1" padding="md">
      ...
      <div style={{
        ...
        <MetricCard
          ...
          icon="⚡"
          subtitle={`${metrics.openOrders} abertos agora`}
        />
      </div>
      ...
      <div style={{ 
        ...
        <Text size="xs" color="secondary">
          Última atualização: {metrics.lastUpdated.toLocaleTimeString('pt-PT')}
        </Text>
        <a 
          href="/analytics" 
          style={{ 
