...
export function useMobileKDS(restaurantId: string): UseMobileKDSReturn {
  ...
  // Determine base URL: if CORE_URL already ends with /rest, use /v1; else use /rest/v1
  ...
  const fetchOrders = useCallback(async () => {
    try {
      ...
      const url = `${baseUrl}/v1/gm_orders?restaurant_id=eq.${restaurantId}&status=in.(OPEN,IN_PREP,READY)&select=*,gm_order_items(*)&order=created_at.asc`;
      const headers = {
  ...
  const startPreparing = useCallback(
    async (ticketId: string) => {
      try {
        ...
        // Update in backend
        const url = `${baseUrl}/v1/gm_orders?id=eq.${ticketId}`;
        await fetch(url, {
          method: "PATCH",
          headers: {
            apikey: CONFIG.CORE_ANON_KEY,
