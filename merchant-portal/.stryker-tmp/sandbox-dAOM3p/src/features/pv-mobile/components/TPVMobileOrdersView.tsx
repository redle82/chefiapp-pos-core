...
export function TPVMobileOrdersView({
  ...
  useEffect(() => {
    ...
    const fetchOrders = async () => {
      ...
      // Query only existing columns: source instead of order_type, id instead of order_number
      const { data } = await dockerCoreClient
        .from("gm_orders")
        .select("id,table_number,source,status,total_cents,created_at")
        .eq("restaurant_id", restaurantId)
        .in("status", ["OPEN", "IN_PREP", "READY"])
        .order("created_at", { ascending: false });

      if (data && Array.isArray(data)) {
        // Map orders: derive order_type from source, generate display number from id
        const mappedOrders = data.map((order: Record<string, unknown>) => {
          const source = (order.source as string) ?? "tpv";
          // Map source to order_type: qr/app → takeaway, delivery → delivery, default → dine_in
          let orderType: ActiveOrder["order_type"] = "dine_in";
          if (source === "delivery") orderType = "delivery";
          else if (source === "qr" || source === "app") orderType = "takeaway";

          return {
            id: order.id as string,
            // Generate display number from last 4 chars of UUID
            order_number: (order.id as string).slice(-4).toUpperCase(),
