/**
 * TEMPORARY: Supabase auth (getSession for pulse check).
 * NOT SOURCE OF TRUTH.
...
const SystemGuardianContext = createContext<
  SystemGuardianContextType | undefined
>(undefined);
...
export const SystemGuardianProvider = ({
  children,
}: {
  ...
  return (
    <SystemGuardianContext.Provider
      value={{ status, errors, isRecovering, reportError, checkPulse }}
    >
      {children}
      {/* Debug Overlay */}
      {isDebugMode() && (
        ...
        >
          <div style={{ color: "white" }}>
            H: {status.health} | BP: {status.tenant} | R: {status.realityStatus}
          </div>
        </div>
      )}
    </SystemGuardianContext.Provider>
