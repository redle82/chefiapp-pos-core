...
export function toUserMessage(err: unknown, fallback: string) {
  ...
  if (err instanceof TypeError) {
    ...
    if (msg.toLowerCase().includes("failed to fetch")) {
      return "Core Offline: Não foi possível ligar ao servidor. Verifique a ligação.";
    }
  }
  ...
  if (err && typeof err === "object" && "message" in err) {
    ...
    if (
      lower.startsWith("failed to read") ||
      lower.startsWith("failed to ") ||
      lower.includes("pgrst") ||
      (lower.includes("status") && lower.includes("response"))
    ) {
      return fallback;
    }
    if (msg) return msg;
