...
export function TPVPOSView() {
  ...
  useEffect(() => {
    ...
    const baseUrl = CONFIG.CORE_URL?.endsWith("/rest")
      ? `${CONFIG.CORE_URL}/v1`
      : `${CONFIG.CORE_URL || ""}/rest/v1`;
    const urlWithAssets = `${baseUrl}/gm_products?select=*,gm_product_assets(image_url)&restaurant_id=eq.${restaurantId}&available=eq.true&order=created_at.asc`;
    const urlPlain = `${baseUrl}/gm_products?select=*&restaurant_id=eq.${restaurantId}&available=eq.true&order=created_at.asc`;
    const headers = {
    ...
    fetch(urlWithAssets, { headers })
      ...
      .then((data) => setProducts(normalizeProducts(data)))      .catch(() => setProducts([]));
