...
export type TPVEventHandler<T = unknown> = (event: TPVCentralEvent<T>) => void | Promise<void>;

export interface TPVCentralEventBus {
    // Subscribe to events
    on<T>(eventType: TPVCentralEventType, handler: TPVEventHandler<T>): () => void;

    // Emit events
    emit<T>(event: TPVCentralEvent<T>): void;

    // Request-response pattern
    request<TReq, TRes>(event: TPVCentralEvent<TReq>, timeoutMs?: number): Promise<TPVCentralEvent<TRes>>;
}
...
class InMemoryEventBus implements TPVCentralEventBus {
    ...
    private pendingRequests: Map<string, { resolve: (e: TPVCentralEvent) => void; timeout: NodeJS.Timeout }> = new Map();
    ...
    emit<T>(event: TPVCentralEvent<T>): void {
        console.log('[TPVCentralEvents] Emit:', event.type, event);

        // Handle response to pending request
        if (event.correlationId && this.pendingRequests.has(event.correlationId)) {
            const { resolve, timeout } = this.pendingRequests.get(event.correlationId)!;
        ...
        if (handlers) {
            handlers.forEach(handler => {
                try {
                    ...
                    console.error('[TPVCentralEvents] Handler error:', err);
                }
            });
        }
    }

    async request<TReq, TRes>(event: TPVCentralEvent<TReq>, timeoutMs = 5000): Promise<TPVCentralEvent<TRes>> {
...
export const TPVCentralEmitters = {
    ...
    reportException(payload: OrderExceptionPayload, waiterId: string): void {
        tpvEventBus.emit(createEvent('order.exception', payload, 'mini-tpv', waiterId));
    },
    ...
    // Operator broadcast
    broadcast(message: string, priority: 'info' | 'warning' | 'urgent', operatorId: string, operatorName: string): void {
        tpvEventBus.emit(createEvent('operator.broadcast', {
            operatorId,
            operatorName,
            message,
            priority,
