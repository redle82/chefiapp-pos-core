...
export const CartDrawer: React.FC = () => {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      if (result.success) {
        setStatus("success");
        // ERRO-001 Fix: Feedback claro com ID do pedido
        const orderIdShort = result.order_id?.slice(-4).toUpperCase();
        setMessage(`✅ Pedido recebido (#${orderIdShort})! Aguarde o preparo.`);
        ...
        if (result.order_id) {
          ...
          }, 2500); // 2.5s para o usuário ler a confirmação
        }
            ...
            <div className="p-6 bg-white/5 border-t border-white/5">
              ...
              {status === "success" && (
                ...
                >
                  ...
                  {orderId &&
                    orderTimestamp &&
                    Date.now() - orderTimestamp < 120000 && (
                      <button
                        onClick={async () => {
                          if (
                            confirm(
