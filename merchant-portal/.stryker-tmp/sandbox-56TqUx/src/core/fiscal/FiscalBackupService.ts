...
export class FiscalBackupService {
   ...
   */
  static async exportFiscals(
    options: FiscalBackupOptions,
  ): Promise<FiscalBackupResult> {
    try {
      const {
        format,
        startDate,
   ...
   */
  static async recoverFiscals(
    options: FiscalRecoveryOptions,
  ): Promise<FiscalRecoveryResult> {
    try {
      const { orderId, fiscalEventId, restaurantId, startDate, endDate } =
        options;
