 ...
 * 4. ?skip_activation=true só com ?debug=1 (pedido explícito)
 *
...
export const RequireActivation = ({ children }: { children: JSX.Element }) => {
  ...
  useEffect(() => {
    const checkActivation = async () => {
      // Route exemption: select-tenant, access-denied
      const currentPath = window.location.pathname;
      if (
        currentPath.startsWith("/app/select-tenant") ||
        currentPath.startsWith("/app/access-denied")
      ) {
        setIsVerified(true);
        return;
      }
      ...
      if (urlParams.get("skip_activation") && isDebugMode()) {
        setIsVerified(true);
        return;
      }
