...
class CoreKernel {
    ...
    public signal(layer: SystemLayer, status: LayerStatus) {
        if (this.locked && status !== 'dead') {
            console.warn(`[CoreKernel] REJECTED signal to ${LayerNames[layer]} (System Locked).`);
            return;
        }
        ...
        if (status === 'stable' && layer > SystemLayer.L0_PHYSICS) {
            ...
            if (previous !== 'stable' && previous !== 'degraded') {
                console.warn(`[CoreKernel] Violation: Layer ${LayerNames[layer]} tried to stabilize, but ${LayerNames[layer - 1]} is ${previous}. Rejecting.`);
        ...
        if (status !== 'stable') {
            console.log(`[CoreKernel] Layer ${LayerNames[layer]} signal: ${status}`);
        }
    }
    ...
    public panic(reason: string) {
        console.error(`[CoreKernel] 🚨 SYSTEM PANIC: ${reason}`);
        this.locked = true;
        this.degraded = true;
        this.notify();
    }
