...
const EXCEPTION_OPTIONS: {
...
}[] = [
  {
    ...
    labelKey: "exceptionReport.itemUnavailable",
    icon: "🚫",
  },
  {
    value: "customer_complaint",
    labelKey: "exceptionReport.customerComplaint",
    icon: "😤",
  },
  { value: "delayed", labelKey: "exceptionReport.delayedOrder", icon: "⏰" },
  {
    value: "payment_issue",
    labelKey: "exceptionReport.paymentIssue",
    icon: "💳",
  },
  { value: "other", labelKey: "exceptionReport.other", icon: "❓" },
];
    ...
    try {
      const payload: OrderExceptionPayload = {
        ...
        description:
          description.trim() ||
          t(
            EXCEPTION_OPTIONS.find((o) => o.value === selectedType)?.labelKey ||
              "",
          ) ||
          "",
      ...
      >
        ...
        >
          <Text size="lg" weight="bold" style={{ color: "white" }}>
            {t("exceptionReport.title")}
          </Text>
          <Text
            size="sm"
            style={{ color: "rgba(255,255,255,0.8)", marginTop: 4 }}
          >
            {t("exceptionReport.tableNumber", { number: tableNumber })}
          </Text>
        </div>
          ...
          >
            {EXCEPTION_OPTIONS.map((option) => (
              ...
              >
                ...
                <Text
                  size="md"
                  weight={selectedType === option.value ? "bold" : "regular"}
                  color="primary"
                >
                  {t(option.labelKey)}
                </Text>
          ...
          <div style={{ marginBottom: spacing[4] }}>
            {items.length > 0 ? (
                  ...
                  {items.map((item) => (
                    ...
                    >
                      <Text
                        size="sm"
                        color={
                          selectedItemId === item.id ? "primary" : "secondary"
                        }
                      >
                        {item.name}
                      </Text>
              ...
              <Text size="sm" color="destructive">
                {t("exceptionReport.noItemsAvailable")}
              </Text>
            )}
          </div>
        )}
        ...
        <Text
          ...
          style={{ marginBottom: spacing[2] }}
        >
          {t("exceptionReport.descriptionLabel")}
        </Text>
        <textarea
          value={description}
          onChange={(e) => setDescription(e.target.value)}
          placeholder={t("exceptionReport.descriptionPlaceholder")}
          style={{
            width: "100%",
            minHeight: 80,
            padding: spacing[3],
        ...
        >
          <Button tone="neutral" onClick={onClose} style={{ flex: 1 }}>
            {t("exceptionReport.cancel")}
          </Button>
          <Button
            tone="destructive"
            ...
            style={{ flex: 1 }}
          >
            {sending ? t("exceptionReport.sending") : t("exceptionReport.send")}
          </Button>
        </div>
      </div>
    </div>
