  ...
  const categories = [
    {
      name: 'Entradas',
      items: [
        { name: 'Pastel de Bacalhau', price: '3,50 €' },
        { name: 'Croquete de Presunto', price: '2,80 €' },
      ],
    },
    {
      name: 'Pratos',
      items: [
        { name: 'Bacalhau à Brás', price: '12,00 €' },
        { name: 'Arroz de Marisco', price: '14,50 €' },
      ],
    },
  ]
  return categories
...
function ProgressItem({ ok, label }: { ok: boolean; label: string }) {
  return (
    <span style={{
      ...
      <span aria-hidden style={{ fontSize: 10 }}>{ok ? '✓' : '○'}</span>
      <span>{label}</span>
    </span>
  )
}
  ...
  return (
    ...
    >
      ...
      <div
        style={{
          flex: '0 0 auto',
          background: `linear-gradient(135deg, ${theme.primaryColor} 0%, ${theme.secondaryColor || theme.primaryColor}${isBold ? 'ff' : 'dd'} 100%)`,
          padding: '14px 12px',
