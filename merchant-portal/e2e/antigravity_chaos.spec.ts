...
test.describe('Phase L.1: Human Journey (Public -> Onboarding)', () => {
    ...
    test('Onboarding Flow (Owner)', async ({ page }) => {
        ...
        // 1. Login (Mock)
        await page.getByPlaceholder('O teu email').fill('test_owner@goldmonkey.com');
        ...
        await page.waitForSelector('text=Publicado', { timeout: 5000 }).catch(() => { }); // Optimistic wait
...
test.describe('Phase L.2/3: Massive Simulation (Scale)', () => {
    ...
    test('Simulate 5 Concurrent Restaurants (Mini-Scale)', async ({ browser }) => {
        // Due to local resource limits, we start with 5.
        // If successful, we can loop this test or use a parameter.

        const RESTAURANTS = Array.from({ length: CONFIG.SCALE.RESTAURANTS }, (_, i) => ({
            id: i + 1,
        ...
        for (const r of RESTAURANTS) {
            ...
            await page.evaluate((profile) => {
                ...
                localStorage.setItem('chefiapp_restaurant_id', `sim_rest_${profile.id}`);
        ...
        for (const r of RESTAURANTS) {
            // Create KDS Context
            const ctx = await browser.newContext({ viewport: CONFIG.VIEWPORTS.TABLET });
        ...
        await Promise.all(contexts.map(async ({ page, profile }, index) => {
            ...
            for (let i = 0; i < 5; i++) {
                ...
                if (!hasItems) {
                    ...
                    await page.getByLabel('Item: nome').fill(`Chaos Burger ${profile.id}`);
                ...
                if (hasItemsNow) {
                    // Click first product
                    await page.locator('.product-card').first().click({ timeout: 2000 }).catch(() => LOG('No product card click'));

                    // Click Charge/Pay
                    await page.getByRole('button', { name: /cobrar/i }).click({ timeout: 2000 }).catch(() => { });

                    // Confirm
                    await page.getByRole('button', { name: /confirmar/i }).click({ timeout: 2000 }).catch(() => { });
