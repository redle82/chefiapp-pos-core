...
// Read credentials robustly using process.cwd() for ESM compatibility
...
test.describe('Sovereign TPV: Financial Core Verification', () => {
    ...
    test.beforeEach(async ({ page }) => {
        ...
        console.log(`🔐 STEP 0: Authentication via /login?debug=1... (User: ${credentials.email})`);
    ...
    test.skip('The Perfect Shift: Open -> Sell -> Pay -> Close', async ({ page }) => {
        ...
        try {
            await expect(page.getByText('Gestão do Menu', { exact: false })).toBeVisible({ timeout: 5000 });
            console.log('✅ Menu Header Found');
        } catch (e) {
            console.log('⚠️ Header "Gestão do Menu" not found. Reloading...');
            await page.reload();
            await expect(page.getByText('Gestão do Menu', { exact: false })).toBeVisible({ timeout: 10000 });
        }
        ...
        if (await openRegisterBtn.isVisible()) {
            ...
            const confirmBtn = page.getByRole('button', { name: 'Abrir Caixa' }).filter({ hasText: /^Abrir Caixa$/ });
            await confirmBtn.click();

            // ASSERTION UPDATE: Verify state change (ABERTO) instead of ephemeral toast
            await expect(page.getByText('ABERTO', { exact: true })).toBeVisible({ timeout: 10000 });
        } else {
        ...
        const productCard = page.getByTestId(/^product-card-/).filter({ hasText: 'Coca Cola SQL' }).first();
        ...
        try {
            ...
            console.log('⚠️ Product "Coca Cola SQL" not found after 20s. Panic Dump:');
            // DEBUG: List available products
            const products = await page.getByTestId(/^product-card-/).allTextContents();
        ...
        if (await openRegisterModal.isVisible()) {
            ...
            await expect(page.getByText('Caixa aberto com sucesso')).toBeVisible({ timeout: 10000 });
        ...
        await expect(page.getByText(/Voltar ao Menu/)).toBeVisible({ timeout: 10000 });
        ...
        // 1. Send to Kitchen
        await page.getByRole('button', { name: /Enviar Cozinha/i }).first().click();

        // 2. Mark Ready (Assuming immediate transition for now or fast mock)
        // We might need to wait for state update if it's async
        await expect(page.getByRole('button', { name: /Marcar Pronto/i }).first()).toBeVisible({ timeout: 10000 });
        await page.getByRole('button', { name: /Marcar Pronto/i }).first().click();

        // 3. Pay
        console.log('💳 STEP 6: Executing Payment...');
        await expect(page.getByRole('button', { name: /Cobrar/i }).first()).toBeVisible({ timeout: 10000 });
        ...
        await expect(page.getByText('Total a Pagar')).toBeHidden({ timeout: 10000 });
        ...
        await expect(page.getByText('FECHADO', { exact: true })).toBeVisible({ timeout: 10000 });
    });
