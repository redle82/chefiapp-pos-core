...
describe('Hardening P0-E: Optimistic Locking Integration', () => {
    ...
    it('SHOULD reject status update after payment increments version', async () => {
        ...
        const orderRes = await pool.query('SELECT version, status FROM gm_orders WHERE id = $1', [testId]);
        ...
        const finalRes = await pool.query('SELECT status FROM gm_orders WHERE id = $1', [testId]);
