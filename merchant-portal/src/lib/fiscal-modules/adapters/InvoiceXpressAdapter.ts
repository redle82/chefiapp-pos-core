...
export class InvoiceXpressAdapter implements FiscalObserver {
  ...
  async onSealed(seal: LegalSeal, event: CoreEvent): Promise<FiscalResult> {
    ...
    const taxDoc = (event.payload as any)?.tax_document as
      | TaxDocument
      | undefined;

    if (!taxDoc) {
      console.warn(
        "[InvoiceXpressAdapter] No tax_document in event payload. Using fallback.",
      );
      ...
      const fallbackVatAmount =
        (totalAmount * fallbackVatRate) / (1 + fallbackVatRate);
  ...
  private async processWithTaxDoc(
    taxDoc: TaxDocument,
    event: CoreEvent,
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      try {
        ...
        console.log(
          `[InvoiceXpressAdapter] ✅ Invoice created successfully (attempt ${attempt})`,
          {
        ...
        return {
          ...
          pdf_url:
            ...
            }.app.invoicexpress.com/documents/${response.id}.pdf`,
        ...
        const isRetriable =
          !isClientError &&
          (isNetworkError ||
            (error.message?.includes("5") && attempt < MAX_RETRIES));
        ...
        if (!isRetriable || attempt === MAX_RETRIES) {
          ...
          console.error(
            `[InvoiceXpressAdapter] ❌ API Error (attempt ${attempt}/${MAX_RETRIES}):`,
            error,
          );
          return {
            status: "REJECTED",
        ...
        console.warn(
          `[InvoiceXpressAdapter] ⚠️ Retry ${attempt}/${MAX_RETRIES} after ${delay}ms:`,
          error.message,
        );
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          errorMessage =
            errorJson.errors?.join(", ") || errorJson.message || errorMessage;
        } catch {
      ...
      if (error.name === "AbortError") {
        throw new Error(
          "Request timeout: InvoiceXpress API did not respond in time",
        );
      }
