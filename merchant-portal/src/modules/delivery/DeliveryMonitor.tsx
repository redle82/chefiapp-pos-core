...
export const DeliveryMonitor: React.FC = () => {
    ...
    const [selectedOrder, setSelectedOrder] = useState<IntegrationOrder | null>(null);
    ...
    useEffect(() => {
        ...
        const channel = supabase.channel(`monitor-delivery-${tenantId}`)
            .on(
                'postgres_changes',
                {
                    event: 'INSERT',
                ...
                <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                        <tr>
                            ...
                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Ref / External ID</th>
                            ...
                            orders.map(order => (
                                <tr key={order.external_id} className="hover:bg-gray-50">
                                    <td className="px-6 py-4 whitespace-nowrap text-gray-500">
                                        ...
                                        <span className="text-xs text-gray-400">{new Date(order.received_at).toLocaleDateString()}</span>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${order.source === 'glovo' ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800'}`}>
                                            {order.source}
                                        </span>
                                    </td>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
                    <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[80vh] flex flex-col">
                        <div className="p-4 border-b flex justify-between items-center">
                            <h3 className="text-lg font-bold">Raw Payload: {selectedOrder.reference}</h3>
                        ...
                        <div className="p-4 border-t bg-gray-50 flex justify-end">
                            <button onClick={() => setSelectedOrder(null)} className="px-4 py-2 bg-gray-200 rounded text-gray-700 hover:bg-gray-300">Close</button>
                        </div>
                    </div>
                </div>
            )}
