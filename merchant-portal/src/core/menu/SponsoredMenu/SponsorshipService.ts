...
export class SponsorshipService {
    ...
    public static async getActiveSponsorships(
        ...
        return new Map(
            visible.map(s => [s.product_id, s as ProductSponsorship])
        );
    }
    ...
    public static async trackEvent(
        ...
        try {
            ...
            if (error) {
                console.warn(`[SponsorshipService] Failed to track ${eventType}:`, error);
            }
        } catch (err) {
    ...
    public static async createCampaign(
        campaign: Omit<ProductSponsorship, 'id' | 'created_at' | 'updated_at' | 'impressions' | 'clicks' | 'conversions' | 'revenue_cents'>
