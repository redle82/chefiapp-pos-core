...
class MenuOptimizationService {
     ...
     */
    async analyzeMenuItems(restaurantId: string, days: number = 30): Promise<MenuItemAnalysis[]> {
        try {
            ...
            for (const [productId, data] of productMap.entries()) {
                ...
                const maxSales = Math.max(...Array.from(productMap.values()).map(p => p.salesCount));
                const popularity = maxSales > 0 ? (data.salesCount / maxSales) * 100 : 0;
    ...
    async suggestPrices(restaurantId: string): Promise<PriceSuggestion[]> {
        ...
        for (const analysis of analyses) {
            ...
            if (analysis.profitMargin < 20 && analysis.popularity > 50) {
                ...
                suggestedPrice *= 1.1;
                reason = 'High demand, low margin - increase price to improve profitability';
