import type { CoreEvent, EventStore, EventType, StreamId } from "../../../../event-log/types";
...
export class BrowserEventStore implements EventStore {
    ...
    async append(event: CoreEvent, expectedStreamVersion?: number): Promise<void> {
        ...
        if (expectedStreamVersion !== undefined) {
            ...
            if (currentVersion !== expectedStreamVersion) {
                throw new Error(`[BrowserEventStore] Concurrency Violation: Expected ${expectedStreamVersion}, got ${currentVersion}`);
            }
        }
    ...
    async readStream(stream_id: StreamId): Promise<CoreEvent[]> {
        ...
        // For MVP/Demo, getting all and filtering is (barely) acceptable but slow.
    ...
    async readAll(filter?: { stream_id?: StreamId; type?: EventType; since?: Date; until?: Date; }): Promise<CoreEvent[]> {
        const all = await GlobalEventStore.getAllSince(filter?.since?.getTime() || 0);
        ...
        if (filter?.until) {
            filtered = filtered.filter(e => (e.occurred_at?.getTime() || e.meta.timestamp) <= filter.until!.getTime());
        }
