 ...
 */
export async function persistOpenCashRegister(context: EffectContext): Promise<void> {
    const { entityId, restaurantId, opening_balance_cents, opened_by, name } = context;
    ...
    if (error) {
        ...
        if (error.message && error.message.includes('CASH_REGISTER_ALREADY_OPEN')) {
            Logger.warn('[Sovereignty] Cash Register already open, skipping projection.');
            return;
        }
        throw new Error(`Open Projection Failed: ${error.message}`);
    }

    Logger.info('[Sovereignty] Cash Register Opened Successfully', { id: result.id });
}
 ...
 */
export async function persistCloseCashRegister(context: EffectContext): Promise<void> {
    ...
    Logger.info('[Sovereignty] Cash Register Closed Successfully', { id: entityId, totalSales: totalSalesCents });
}
