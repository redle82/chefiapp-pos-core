...
export const sendPulse = async (context?: PulseContext) => {
  if (getBackendType() !== BackendType.docker) {
    return {
      ...
      status: "healthy",
      vitality: {
        active_pilot: context?.tenantSlug || "unknown",
        menu_items: 0,
        orders_today: 0,
        revenue_today: 0,
      },
  ...
  const ordersRes = await core
    .from("gm_orders")
    .select("total, created_at")
    .then((r) => r);
  const allOrders = (Array.isArray(ordersRes.data) ? ordersRes.data : []) as {
    total?: number;
    created_at?: string;
  }[];
  ...
  const signal = {
    ...
    vitality: {
      active_pilot:
        tenantSlug === "sofia-gastrobar" ? "Sofia Gastrobar" : tenantSlug,
  ...
  if (insertRes.error) {
    Logger.warn("[EMPIRE_PULSE] Failed to transmit pulse:", {
      error: insertRes.error,
    });
  } else {
    Logger.debug(
      `[EMPIRE_PULSE] Signal Transmitted @ ${new Date().toLocaleTimeString()} [${tenantSlug}]`,
      { payload: signal },
    );
  }
...
export const emitPulseEvent = async (event: any) => {
  ...
  const res = await core
    ...
    .insert({
      ...
      events: [event],
      risk: {
        level: event.severity === "critical" ? "high" : "low",
        active_alerts: 1,
      },
    })
    .then((r) => r);
  ...
  if (res.error) {
    Logger.warn("[EMPIRE_PULSE] Failed to transmit event:", {
      error: res.error,
    });
  } else {
    Logger.debug(`[EMPIRE_PULSE] 🚨 EVENT TRANSMITTED: ${event.signal}`, {
      event,
    });
  }
};
