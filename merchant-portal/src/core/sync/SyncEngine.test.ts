...
import { calculateNextRetry } from './RetryStrategy';
...
describe('SyncEngine Stress Test', () => {
    ...
    it('should process pending items when online', async () => {
        // Mock Pending Items
        const pendingItems = [
            { id: '1', type: 'ORDER_CREATE', payload: { items: [{ id: 'p1' }] }, attempts: 0, status: 'queued' }
        ];
        (IndexedDBQueue.getPending as any).mockResolvedValue(pendingItems);
        (supabase.rpc as any).mockResolvedValue({ data: { id: 'real_1' }, error: null });
        ...
        expect(supabase.rpc).toHaveBeenCalledWith('create_order_atomic', expect.any(Object));
    ...
    it('should handle sync error and schedule retry', async () => {
        // Mock Pending Items
        const pendingItems = [
            { id: '2', type: 'ORDER_CREATE', payload: { items: [{ id: 'p1' }] }, attempts: 0, status: 'queued' }
        ];
        (IndexedDBQueue.getPending as any).mockResolvedValue(pendingItems);
        // Mock RPC Error
        ...
        // Should verify it called updateStatus with 'failed' and nextRetry
        ...
        const pendingItems = [
            {
                id: '3',
                type: 'ORDER_UPDATE',
                payload: { orderId: 'real_1', restaurantId: 'rest_1', action: 'add_item', items: [{ id: 'p2', price: 10, quantity: 1, name: 'Burger' }] },
                attempts: 0,
                status: 'queued'
            }
        ];
    ...
    it('should update listeners on state change', () => {
        ...
        // Initial call
        expect(listener).toHaveBeenCalledWith(expect.objectContaining({ isProcessing: false }));
        ...
        window.dispatchEvent(new Event('offline'));
        expect(listener).toHaveBeenCalledWith(expect.objectContaining({ networkStatus: 'offline' }));

        window.dispatchEvent(new Event('online'));
        expect(listener).toHaveBeenCalledWith(expect.objectContaining({ networkStatus: 'online' }));
