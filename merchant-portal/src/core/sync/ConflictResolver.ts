...
export const ConflictResolver = {
     ...
     */
    async shouldApplyUpdate(table: string, id: string, localTimestamp: number): Promise<boolean> {
        try {
            ...
            if (error) {
                Logger.warn(`[ConflictResolver] Failed to check remote state for ${table}:${id}`, error);
            ...
            if (!isNewer) {
                Logger.info(`[ConflictResolver] Updates for ${table}:${id} are stale. Local: ${localTimestamp}, Remote: ${remoteTime}. Skipping.`);
            }
