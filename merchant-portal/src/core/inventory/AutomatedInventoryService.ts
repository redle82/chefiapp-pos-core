...
class AutomatedInventoryService {
        ...
        try {
            ...
            for (const item of items || []) {
                ...
                // Low stock
                    ...
                    const urgency = currentStock <= minThreshold * 0.5 ? 'high' : 'medium';
                    alerts.push({
                        itemId: item.id,
                        itemName: item.name,
                        type: 'low_stock',
     ...
     */
    async getReorderSuggestions(restaurantId: string): Promise<Array<{
        itemId: string;
        itemName: string;
        currentStock: number;
     ...
     */
    async createReorderTask(restaurantId: string, itemId: string, quantity: number): Promise<void> {
        try {
            // This would integrate with AppStaff to create a task
            // For now, just log
            Logger.info('Reorder task created', { restaurantId, itemId, quantity });
        } catch (err) {
            Logger.error('Failed to create reorder task', err, { restaurantId, itemId, quantity });
        }
    }
}
