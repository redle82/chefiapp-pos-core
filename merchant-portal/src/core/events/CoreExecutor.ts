...
export const CoreExecutor = {
  ...
  reduce(state: SystemState, event: EventEnvelope): SystemState {
    ...
    if (event.stream_version && event.stream_version < 0) {
      ...
      Logger.warn(
        `[CoreExecutor] Stream Version ${event.stream_version} observed but strict check pending State Schema migration.`,
      );
    }
    ...
    switch (event.type) {
      ...
      case "ORDER_UPDATED": {
        const targetIndex = nextState.orders.findIndex(
        ...
        else if (["cancel", "void"].includes(action))
  ...
  reduceAll(
    ...
    return sorted.reduce(
      (state, event) => this.reduce(state, event),
      initialState,
    );
  },
};
