...
class DiagnosticEngineClass {
    ...
    public emit(
        ...
        try {
            ...
            const event: DiagnosticEvent = {
                code, title, metricType, userMessage,
    ...
    private sanitize(data: any): any {
        ...
        try {
            const str = JSON.stringify(data, (key, value) => {
                ...
                if (/token|password|secret|key|authorization/i.test(key)) return '***REDACTED***';
                return value;
            });
    ...
    private logConsole(event: DiagnosticEvent) {
        const style =
            event.metricType === 'error' ? 'color: red; font-weight: bold' :
                event.metricType === 'warning' ? 'color: orange' : 'color: blue';
...
export const DiagnosticEngine = {
    emit: (...args: Parameters<DiagnosticEngineClass['emit']>) => getDiagnosticEngine().emit(...args),
