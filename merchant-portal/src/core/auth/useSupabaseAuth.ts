...
export function useSupabaseAuth(): SupabaseAuthState {
  ...
  useEffect(() => {
    ...
    // Docker backend: não chamar Supabase Auth; retornar sem sessão
    ...
    if (typeof window !== "undefined") {
      ...
      if (import.meta.env.DEV && isDemo) {
        console.warn(
          "[Auth] 🚧 DEV BYPASS: Activating Demo Mode (Mock Session)"
        );
        const mockUser = {
          id: "demo-user",
          aud: "authenticated",
    ...
    if (typeof window !== "undefined") {
      ...
      if (hash && hash.includes("access_token")) {
        ...
        authClient.auth
          .getSession()
          .then(({ data: { session: hashSession }, error: hashError }) => {
            if (!mounted) return;
    ...
    if (!hashProcessed) {
      ...
      setTimeout(() => {
        if (mounted && !initializedRef.current) {
          console.warn(
            "[Auth] Hash processed but no session after 3s - setting loading to false"
          );
          setLoading(false);
          initializedRef.current = true;
        }
