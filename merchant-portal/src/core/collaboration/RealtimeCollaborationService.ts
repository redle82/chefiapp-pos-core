...
class RealtimeCollaborationService {
        ...
        try {
            ...
            this.presenceChannel = supabase.channel(`presence:${restaurantId}`)
                ...
                .on('presence', { event: 'join' }, ({ key, newPresences }) => {
                    console.log('[RealtimeCollaboration] User joined:', key, newPresences);
                })
            ...
            Logger.error('Failed to initialize presence', err, { restaurantId, userId });
        }
    }
    ...
    async broadcastEvent(event: CollaborationEvent): Promise<void> {
        try {
            ...
            await supabase.channel(`collaboration:${event.resource}`)
                .send({
                    type: 'broadcast',
                    event: 'action',
                    payload: event,
