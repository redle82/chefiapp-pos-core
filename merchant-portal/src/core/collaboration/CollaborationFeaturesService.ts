...
class CollaborationFeaturesService {
    ...
    async sendChatMessage(
        ...
        try {
            const userId = realtimeCollaborationService.getActiveUsers()[0]?.userId || 'unknown';
            const userName = realtimeCollaborationService.getActiveUsers()[0]?.userName || 'User';
            ...
            Logger.error('Failed to send chat message', err, { restaurantId, message });
            return null;
        }
    }
     ...
     * Get chat messages
     */
    async getChatMessages(restaurantId: string, resourceId?: string, limit: number = 50): Promise<ChatMessage[]> {
        try {
            let query = supabase
                .from('chat_messages')
                .select('*')
