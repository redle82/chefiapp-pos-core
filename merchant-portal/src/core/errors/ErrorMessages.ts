...
export function getErrorMessage(error: any, context?: ErrorContext): string {
    ...
    if (code === 'TABLE_HAS_ACTIVE_ORDER' || message.includes('TABLE_HAS_ACTIVE_ORDER')) {
    ...
    if (code === '23505' && message.includes('idx_one_open_order_per_table')) {
        return 'Já existe um pedido ativo para esta mesa. Use o pedido existente.';
    }

    // Payment errors
    if (code === 'PAYMENT_FAILED' || message.includes('payment') || message.includes('pagamento')) {
 ...
 */
export function getErrorSuggestion(error: any, context?: ErrorContext): string | null {
    const code = error?.code || context?.code;
    const message = error?.message || context?.message || '';
