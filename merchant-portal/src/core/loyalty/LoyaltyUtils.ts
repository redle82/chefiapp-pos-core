...
export const getCustomerTier = (totalSpendCents: number = 0): TierConfig => {
  if (totalSpendCents >= LOYALTY_TIERS.platinum.thresholdCents)
    return LOYALTY_TIERS.platinum;
  if (totalSpendCents >= LOYALTY_TIERS.gold.thresholdCents)
    return LOYALTY_TIERS.gold;
  if (totalSpendCents >= LOYALTY_TIERS.silver.thresholdCents)
...
export const getSmartSuggestion = (
  totalSpendCents: number = 0,
): AICopilotSuggestion | null => {
  ...
  if (
    ...
    progress.remainingCents < 1500
  ) {
    // Less than 15 EUR to go
    const euros = (progress.remainingCents / 100).toFixed(2);
    return {
      type: "loyalty_booster",
      message: `🥈 Faltam apenas ${euros}${getCurrencySymbol()} para nível ${
        progress.nextTierName
      }! Sugira uma sobremesa.`,
