...
const ContextEngineContext = createContext<ContextEngineValue | undefined>(undefined);
...
export function ContextEngineProvider({
    ...
    const contextState = useMemo(() => {
        const base = resolveContext(activeRole, device, isViewMode, originalRole);
        return { ...base, currentTurn };
    }, [activeRole, device, isViewMode, originalRole, currentTurn]);

    const switchView = useCallback((targetRole: UserRole | null) => {
        // Security check: Only Owner (or maybe Manager) can switch views
        if (userRole !== 'owner' && userRole !== 'manager') {
            console.warn('[ContextEngine] Access Denied: Only Owner/Manager can switch views.');
            return;
        }
