...
export function useActivationTracking(): UseActivationTrackingResult {
  ...
  const trackPanelOpen = useCallback(
    (variant: "full" | "compact" | "minimal", count: number) => {
  ...
  const trackClick = useCallback(
    (recommendation: ActivationRecommendation) => {
      if (!recommendation.action?.to) {
        Logger.warn(
          `[useActivationTracking] Click without action.to: ${recommendation.id}`,
        );
        return;
      }
 ...
 * ```
 */
export function useAutoActivationTracking(
  options: UseAutoTrackingOptions,
): UseActivationTrackingResult {
