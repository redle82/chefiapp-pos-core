...
export function useActivationAdvisor(): UseActivationAdvisorResult {
    ...
    const result = useMemo(() => {
        ...
        try {
            ...
            const recommendations = ActivationAdvisor.getActivationRecommendations(input);
            const highPriority = ActivationAdvisor.getHighPriorityRecommendations(input);
            ...
            console.error('[useActivationAdvisor] Error computing recommendations:', e);
            return {
                recommendations: [],
                highPriority: [],
                summary: { high: 0, medium: 0, low: 0, total: 0 },
 ...
 */
function extractAnswersFromBlueprint(blueprint: any): Partial<OnboardingAnswers> {
    ...
    try {
        ...
        if (blueprint.organization) {
            ...
            const typeMap: Record<string, 'restaurant' | 'bar' | 'cafe' | 'club' | 'dark_kitchen'> = {
            ...
            if (blueprint.organization.businessType) {
                answers.restaurant_type = typeMap[blueprint.organization.businessType] || 'restaurant';
            }
        }
        ...
        if (blueprint.operation) {
            ...
            if (blueprint.operation.teamSize) {
                answers.team_size = teamSizeMap[blueprint.operation.teamSize] || 'small';
            }
        }
        ...
        if (blueprint.systemProfiles?.workflowProfile) {
            answers.has_tables = blueprint.systemProfiles.workflowProfile.enableTableService !== false;
        }
