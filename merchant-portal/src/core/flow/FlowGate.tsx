    ...
    const checkFlow = async () => {
      ...
      try {
        ...
        if (isDocker) {
          ...
          if (debug)
            Logger.debug(
              `[FlowGate] Fetching membership for ${session.user.id}`,
            );
          ...
          if (memberError) {
            ...
            if (debug)
              Logger.warn("[FlowGate] Member check error (fallback):", {
                error: memberError,
              });
            const fallbackId =
              getActiveTenant() ||
          ...
          try {
            ...
            if (restaurant) {
              ...
              if (isDocker && typeof window !== "undefined") {
                if (debug)
                  Logger.warn(
                    `[FlowGate] Restaurant not found (404), clearing invalid id: ${restaurantId}`,
                  );
                clearActiveTenant();
                hasOrg = false;
        ...
        Logger.error("[FlowGate] Error:", err);
        if (mounted) {
          clearLoadingTimeout();
          setIsChecking(false);
        }
