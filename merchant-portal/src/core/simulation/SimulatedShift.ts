...
export const runSimulatedShift = async (actions: SimulatorActions) => {
  Logger.info("🎭 STARTING SIMULATION: Friday Night Rush (Compressed)");

  const delay = (ms: number) => new Promise((res) => setTimeout(res, ms));

  // 1. STATE: IDLE (Recovery Mode)
  // ------------------------------------------------
  Logger.debug("Scene 1: The Calm Before the Storm");
  actions.clearOrders();
  ...
  // ------------------------------------------------
  Logger.debug("Scene 2: First Guests Arrive");
  const order1: Order = {
    id: "sim-ord-001",
    tableNumber: 5,
    status: "new",
  ...
  // ------------------------------------------------
  Logger.debug("Scene 3: THE RUSH (High Pressure)");
  ...
  // ------------------------------------------------
  Logger.debug("Scene 4: Clearing the Board");
  ...
  Logger.info("🎭 SCENE FINISHED. The System Breathed.");
};
