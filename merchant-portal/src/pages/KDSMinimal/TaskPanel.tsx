...
import { acknowledgeTask, resolveTask } from '../../core-boundary/writers/TaskWriter';
...
export function TaskPanel({ restaurantId, station, onTaskAcknowledged }: TaskPanelProps) {
  const [tasks, setTasks] = useState<CoreTask[]>([]);
  const [loading, setLoading] = useState(true);
  const [acknowledging, setAcknowledging] = useState<string | null>(null);
  const [evidenceInput, setEvidenceInput] = useState<Record<string, string>>({}); // taskId -> value
  ...
  if (tasks.length === 0) {
    return (
      <div style={{ padding: '16px', backgroundColor: '#f0fdf4', borderRadius: '8px', border: '1px solid #86efac' }}>
        <div style={{ fontWeight: 'bold', marginBottom: '4px' }}>✅ Sem tarefas pendentes</div>
                ...
                <div style={{ 
                  ...
                  {task.station && (
                    <span style={{ fontSize: '10px', color: '#666' }}>
                      {task.station === 'KITCHEN' ? '🍳' : task.station === 'BAR' ? '🍺' : '👨‍💼'} {task.station}
                    </span>
                  )}
                </div>
                ...
                {task.context && (
                  <div style={{ fontSize: '12px', color: '#666' }}>
                    {task.context.delay_seconds && (
                      <span>Atraso: {Math.round(task.context.delay_seconds / 60)} min</span>
                    )}
                    {task.context.table_number && (
                      <span style={{ marginLeft: '8px' }}>Mesa: {task.context.table_number}</span>
                    )}
                ...
                {task.context?.required_evidence === 'TEMP_LOG' && (
                  <div style={{ marginTop: '8px' }}>
                    ...
                    <input
                      ...
                      onChange={(e) => setEvidenceInput(prev => ({ ...prev, [task.id]: e.target.value }))}
                      placeholder="Ex: 4.5"
                      style={{
                        padding: '4px 8px',
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', alignItems: 'flex-end' }}>
                <div style={{ display: 'flex', gap: '8px' }}>
                  ...
                  <button
                    ...
                    disabled={acknowledging === task.id || (task.context?.required_evidence === 'TEMP_LOG' && !evidenceInput[task.id])}
                    style={{
                      padding: '6px 12px',
                      ...
                      border: 'none',
                      borderRadius: '4px',
                      cursor: (acknowledging === task.id || (task.context?.required_evidence === 'TEMP_LOG' && !evidenceInput[task.id])) ? 'not-allowed' : 'pointer',
                      fontSize: '12px',
                      fontWeight: 'bold',
                      opacity: (acknowledging === task.id || (task.context?.required_evidence === 'TEMP_LOG' && !evidenceInput[task.id])) ? 0.5 : 1,
                    }}
                  >
                    ✓ Resolver
                  </button>
