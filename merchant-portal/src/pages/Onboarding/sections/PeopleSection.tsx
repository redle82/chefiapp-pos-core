...
export function PeopleSection() {
  ...
  const [newPerson, setNewPerson] = useState({ name: '', email: '', role: 'staff' as Person['role'] });
  ...
  useEffect(() => {
    ...
    const hasManager = people.some((p) => p.role === 'manager' || p.role === 'owner');
    const isValid = people.length >= 1 && hasManager;

    const status = isValid ? 'COMPLETE' : people.length > 0 ? 'INCOMPLETE' : 'NOT_STARTED';
    ...
    if (isValid && identity.id) {
      ...
      saveTimeoutRef.current = setTimeout(async () => {
        ...
        try {
          ...
          console.log('✅ Pessoas configuradas (implementação completa requer sistema de convites)');
        } catch (error) {
  ...
  return (
    <div style={{ padding: '48px', maxWidth: '800px', margin: '0 auto' }}>
      <h1 style={{ fontSize: '24px', fontWeight: 600, marginBottom: '8px' }}>
        👥 Pessoas {isSaving && <span style={{ fontSize: '14px', color: '#667eea' }}>(Salvando...)</span>}
      </h1>
      ...
      <div style={{ marginBottom: '32px', padding: '16px', backgroundColor: '#f8f9fa', borderRadius: '8px' }}>
        <h3 style={{ fontSize: '16px', fontWeight: 600, marginBottom: '16px' }}>Adicionar Pessoa</h3>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          ...
          <button
            ...
            style={{
              ...
              fontSize: '14px',
              fontWeight: 600,
              cursor: newPerson.name && newPerson.email ? 'pointer' : 'not-allowed',
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
        {people.length === 0 ? (
          <p style={{ fontSize: '14px', color: '#999', fontStyle: 'italic', textAlign: 'center', padding: '32px' }}>
            ...
            >
              <div>
                ...
                <div style={{ fontSize: '12px', color: '#667eea', marginTop: '4px' }}>
                  {person.role === 'owner' ? 'Proprietário' : person.role === 'manager' ? 'Gerente' : 'Funcionário'}
                </div>
              </div>
              <button
      ...
      <div style={{ marginTop: '32px', padding: '16px', backgroundColor: '#f8f9fa', borderRadius: '8px' }}>
        <div style={{ fontSize: '14px', fontWeight: 600, marginBottom: '12px' }}>Checklist:</div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {[
            { label: 'Pelo menos 1 pessoa adicionada', done: people.length >= 1 },
            { label: 'Pelo menos 1 gerente ou proprietário', done: people.some((p) => p.role === 'manager' || p.role === 'owner') },
          ].map((item) => (
            <div key={item.label} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
              <span style={{ fontSize: '16px' }}>{item.done ? '✅' : '⏳'}</span>
              <span style={{ fontSize: '14px', color: item.done ? '#28a745' : '#666' }}>
                {item.label}
              </span>
            </div>
          ))}
