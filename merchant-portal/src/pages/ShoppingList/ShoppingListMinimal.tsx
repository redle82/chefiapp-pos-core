...
export function ShoppingListMinimal() {
  ...
  }, [finalRestaurantId, loadingIdentity]);
  ...
  const handleConfirmPurchase = async (item: ShoppingListItem) => {
    ...
    try {
      ...
      if (result.tasks_closed > 0) {
        console.log(`✅ Compra confirmada! ${result.tasks_closed} tarefa(s) fechada(s).`);
      }
    ...
    <div style={{ padding: '24px', maxWidth: '1200px', margin: '0 auto' }}>
      ...
      {items.length === 0 ? (
        <div style={{ 
          ...
          <p style={{ fontSize: '18px', marginBottom: '8px' }}>✅ Estoque em dia!</p>
          <p style={{ fontSize: '14px' }}>
            Todos os ingredientes estão acima do mínimo. Nenhuma compra necessária no momento.
          </p>
        </div>
      ) : (
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: '14px', color: '#666', marginBottom: '8px' }}>
                    <div>📊 Atual: <strong>{item.current_qty} {item.unit}</strong></div>
                    <div>📉 Mínimo: <strong>{item.min_qty} {item.unit}</strong></div>
                    <div>📦 Déficit: <strong>{item.deficit} {item.unit}</strong></div>
                  </div>
                  ...
                  <div style={{ 
                    ...
                    💡 Sugestão: Comprar <strong>{item.suggested_qty} {item.unit}</strong>
                  </div>
                  ...
                  <button
                    ...
                    style={{
                      padding: '10px 20px',
                      backgroundColor: confirming === item.ingredient_id ? '#9ca3af' : '#10b981',
                      color: '#fff',
                      border: 'none',
                      borderRadius: '6px',
                      cursor: confirming === item.ingredient_id ? 'not-allowed' : 'pointer',
                      fontSize: '14px',
                      fontWeight: 'bold',
                      width: '100%',
                    }}
                  >
                    {confirming === item.ingredient_id ? '⏳ Confirmando...' : '✅ Comprei'}
                  </button>
                </div>
              </div>
                  ...
                  <div style={{
                    ...
                    <div style={{ marginBottom: '16px' }}>
                      ...
                      <input
                        type="number"
                        value={confirmQty}
                        onChange={(e) => setConfirmQty(parseFloat(e.target.value) || 0)}
                        min="0"
                        step="0.1"
                        style={{
                          width: '100%',
                    ...
                    <div style={{ display: 'flex', gap: '8px' }}>
                      ...
                      <button
                        ...
                        disabled={confirmQty <= 0 || confirming === item.ingredient_id}
                        style={{
                          flex: 1,
                          padding: '10px',
                          backgroundColor: confirming === item.ingredient_id ? '#9ca3af' : '#10b981',
                      ...
                      >
                        {confirming === item.ingredient_id ? 'Confirmando...' : 'Confirmar'}
                      </button>
                    </div>
                  </div>
                </div>
