...
export function WizardPage() {
    const envInternalToken =
        typeof import.meta.env.VITE_INTERNAL_API_TOKEN === 'string' ? import.meta.env.VITE_INTERNAL_API_TOKEN : undefined
    const envRestaurantId = typeof import.meta.env.VITE_RESTAURANT_ID === 'string' ? import.meta.env.VITE_RESTAURANT_ID : undefined
    const envDefaultSlug =
        (typeof import.meta.env.VITE_DEFAULT_SLUG === 'string' && import.meta.env.VITE_DEFAULT_SLUG) ||
        (typeof import.meta.env.VITE_SLUG === 'string' && import.meta.env.VITE_SLUG) ||
        undefined
    const envMenuItemId = typeof import.meta.env.VITE_MENU_ITEM_ID === 'string' ? import.meta.env.VITE_MENU_ITEM_ID : undefined
    ...
    const envShowDev = import.meta.env.DEV;
    const [showDevConfig, setShowDevConfig] = useState(envShowDev);
    const initialApiBase = CONFIG.API_BASE;
    const [apiBase, setApiBase] = useState<string>(initialApiBase)
    const [internalToken, setInternalToken] = useState<string>(envInternalToken ?? 'dev-token')
    const [restaurantId, setRestaurantId] = useState<string>(envRestaurantId ?? '')
    ...
    const [stripePk, setStripePk] = useState(import.meta.env.VITE_STRIPE_PK || '')
    const [stripeSk, setStripeSk] = useState(import.meta.env.VITE_STRIPE_SK || '')
    const [stripeWhsec, setStripeWhsec] = useState(import.meta.env.VITE_STRIPE_WHSEC || '')

    const [designLevel, setDesignLevel] = useState<'BASIC' | 'PRO' | 'EXPERIENCE'>('PRO')
    const [designTheme, setDesignTheme] = useState<'minimal' | 'light' | 'dark'>('minimal')
    const [designSlug, setDesignSlug] = useState(envDefaultSlug ?? 'sofia-gastrobar')
    ...
    const stepState = useMemo(() => {
        const completed = new Set<number>((Array.isArray(stateJson?.completed_steps) ? stateJson.completed_steps : []).map((n: any) => Number(n)))
    ...
    const stepperSteps = useMemo(() => {
        ...
        return [
            { id: '1', label: 'Identidade', isCompleted: completed.has(1) || step.identity, isActive: current === 1 },
            { id: '2', label: 'Menu', isCompleted: completed.has(2) || step.menu, isActive: current === 2 },
            { id: '3', label: 'Pagamentos', isCompleted: completed.has(3) || step.payments, isActive: current === 3 },
            { id: '4', label: 'Design', isCompleted: completed.has(4) || step.design, isActive: current === 4 },
            { id: '5', label: 'Publish', isCompleted: completed.has(5) || step.published, isActive: current === 5 },
        ]
    ...
    function formatApiError(err: ApiError) {
        ...
        if (err.status === 403 && code === 'WEB_LEVEL_BLOCKED') {
            ...
            return {
                ...
                message: webLevel ? `O nível ${webLevel} não está disponível no seu plano.` : 'Este nível não está disponível no seu plano.',
        ...
        if (err.status === 409 && code === 'GATEWAY_NOT_CONFIGURED') {
            return {
                ...
                message: 'Liga o Stripe (Passo 3) para conseguir criar pedidos com pagamento.',
        ...
        if (err.status === 400) {
            const selling = {
                IDENTITY_INCOMPLETE: 'Completa a Identidade (Passo 1) antes de publicar.',
                ...
                MENU_ITEM_NOT_FOUND: 'O item do menu não existe (confere o MENU_ITEM_ID do seed).',
                MIXED_CURRENCY_NOT_SUPPORTED: 'O pedido tem moedas diferentes. Usa apenas EUR no demo.',
            } as const
        ...
        return { title: 'Erro', message: msg || `HTTP_${err.status} `, detail: '', raw: { status: err.status, body } }
    }
    ...
    const publicUrl = useMemo(() => {
        ...
        return base ? `${base}/public/${encodeURIComponent(s)}` : `/public/${encodeURIComponent(s)}`
    ...
    const publicEnabledReason = useMemo(() => {
        ...
        if (gates.ok === false) return 'Bloqueado pelo plano — faz upgrade para abrir ao público.'
        return ''
    ...
    return (
        <div style={{
            ...
            <div style={{
                ...
                ⚠️ FERRAMENTA INTERNA DEV — NÃO É PRODUTO ⚠️
                <div style={{ fontSize: '13px', fontWeight: 'normal', marginTop: '4px', opacity: 0.95 }}>
                    Wizard real: <code style={{ background: 'rgba(0,0,0,0.2)', padding: '2px 6px', borderRadius: '4px' }}>/app/setup</code>
                </div>
            </div>
            ...
            <header style={{
                ...
                <div>
                    <Text size="xs" color="tertiary" style={{ textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: spacing.xs }}>
                ...
                <div style={{ display: 'flex', gap: spacing.sm, flexWrap: 'wrap' }}>
                    <Button
                        ...
                        onClick={() =>
                            run('load_state', async () => {
                                ...
                                const r = await fetchJson(apiBase, `/internal/wizard/${encodeURIComponent(restaurantId)}/state`, {
                                    method: 'GET',
                    ...
                    {publicUrl && (
                        publicEnabledReason ? (
                            ...
                            <Button variant="outline" as="a" href={publicUrl} target="_blank" rel="noreferrer">
                                Abrir página pública
                            </Button>
                        )
                    )}
            ...
            {import.meta.env.DEV && (
                <Card style={{ marginBottom: spacing.lg }}>
                    ...
                    >
                        <Text size="lg" weight="semibold">Config (DEV)</Text>
                        <Text size="sm" color="secondary">{showDevConfig ? '▼' : '▶'}</Text>
                    </div>
                    {showDevConfig && (
            ...
            <Card style={{ marginBottom: spacing.lg }}>
                ...
                <Button
                    variant="primary"
                    onClick={() => run('identity_manual', async () => {
