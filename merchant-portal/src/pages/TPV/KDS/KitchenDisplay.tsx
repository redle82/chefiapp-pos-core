...
const TicketCard = ({ ticket, onAdvance, isUnseen = false, isSelected = false, onTicketClick, id }: TicketCardProps) => {
    const isNew = ticket.status === 'new';

    // Ticket "Paper" Feel
    const bg = isNew ? Colors.kds.ticket.new : Colors.kds.ticket.standard;
    const activeBorderColor = isNew ? Colors.risk.medium : Colors.info; // Gold for New, Blue for Prep
    ...
    return (
        <motion.div
            ...
            style={{
                ...
                overflow: 'hidden',
                boxShadow: isUnseen
                    ? '0 0 20px rgba(255, 193, 7, 0.6), 0 4px 6px -1px rgba(0, 0, 0, 0.1)'
        ...
        >
            {/* NOVO Badge for unseen orders */}
            {isUnseen && (
                <div className="kds-badge-new" style={{
                    position: 'absolute',
                    top: '-8px',
                    right: '-8px',
                    background: '#ef4444',
            ...
            <div style={{ padding: Spacing.lg }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: Spacing.sm }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <OrderTimer createdAt={typeof ticket.createdAt === 'string' ? ticket.createdAt : ticket.createdAt.toISOString()} />
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: Spacing.sm }}>
                    {ticket.items.map(item => (
                        <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                {item.notes && <span style={{ color: Colors.risk.high, fontSize: '0.8em', fontStyle: 'italic', marginRight: '8px' }}>{item.notes}</span>}
                                <ItemTimer
            ...
            <button
                ...
                style={{
                    ...
                    border: 'none',
                    cursor: 'pointer',
                    background: isNew ? 'rgba(0,0,0,0.2)' : 'rgba(0,0,0,0.2)', // Darken slightly
                    color: 'inherit',
                    fontWeight: 'bold',
                    textTransform: 'uppercase',
                    fontSize: '0.9em',
                    marginTop: Spacing.sm,
                    boxShadow: isSelected ? `0 0 10px ${Colors.info}` : 'none'
                }}>
                {isNew ? 'Iniciar Preparo →' : 'Marcar Pronto ✓'}
            </button>
        </motion.div>
    );
};
...
const MiseEnPlaceMode = () => {
    ...
    return (
        <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', margin: Spacing.lg }}
        >
            <EmptyState
...
export default function KitchenDisplay({ initialStation = 'ALL' }: { initialStation?: KDSStation }) {
    ...
    useEffect(() => {
        document.title = isKDSEffectivelyOffline ? '⚠️ OFFLINE — KDS' : 'ChefIApp POS — KDS';
    ...
    useBumpBar({
        onNavigate: (dir) => {
            ...
            const currentIndex = activeTickets.findIndex(o => o.id === selectedOrderId);
    ...
    return (
        <div style={{
            ...
            <header style={{
                height: '64px',
                borderBottom: `1px solid ${isKDSEffectivelyOffline ? Colors.risk.high : Colors.kds.border}`,
                display: 'flex',
                alignItems: 'center',
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: Spacing.lg }}>
                    <div style={{
                        ...
                        height: '12px',
                        borderRadius: '50%',
                        background: isKDSEffectivelyOffline ? Colors.risk.high : (hasPressure ? Colors.risk.high : Colors.risk.low),
                        boxShadow: `0 0 10px ${isKDSEffectivelyOffline ? Colors.risk.high : (hasPressure ? Colors.risk.high : Colors.risk.low)}`,
                        animation: isKDSEffectivelyOffline ? 'pulse-fast 1s infinite' : 'none'
                    }} />
                    <h1 style={{ fontSize: Typography.h3.fontSize, fontWeight: 'bold', letterSpacing: '0.2em', textTransform: 'uppercase', color: Colors.kds.text.secondary }}>
                        {isKDSEffectivelyOffline ? '🚧 OFFLINE — AGUARDANDO CONEXÃO 🚧' : (hasPressure ? 'Produção' : 'Mise en Place')}
                    </h1>
                    {/* === UNSEEN ORDERS BADGE === */}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: Spacing.lg }}>
                    ...
                    {/* === KDS HARDENING: Status de Conexão Detalhado === */}
                    <div style={{ display: 'flex', alignItems: 'center', gap: Spacing.sm }}>
                        {isKDSEffectivelyOffline ? (
                            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', color: Colors.risk.high }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', color: Colors.risk.medium }}>
                    ...
                    <span style={{ fontFamily: 'monospace', fontSize: Typography.h3.fontSize, fontWeight: 'bold', color: Colors.kds.text.dim }}>
                    ...
                    {!hasPressure ? (
                        ...
                        >
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
                                <h2 style={{ fontSize: Typography.uiTiny.fontSize, fontWeight: 'bold', color: Colors.risk.medium, marginBottom: Spacing.lg, textTransform: 'uppercase', letterSpacing: '0.2em', display: 'flex', justifyContent: 'space-between' }}>
                                    Novos Pedidos <span>{newOrders.length}</span>
                                </h2>
                                <div style={{ flex: 1, overflowY: 'auto', paddingRight: Spacing.sm, paddingBottom: Spacing['4xl'] }}>
                                    {newOrders.map(t => (
                                        <TicketCard
                                            key={t.id}
                                            ticket={t}
                                    ...
                                    {newOrders.length === 0 && (
                                        <div style={{ color: Colors.kds.text.dim, textAlign: 'center', paddingTop: Spacing['2xl'], fontStyle: 'italic' }}>A aguardar entrada...</div>
                                    )}
                                </div>
                            </div>

                            {/* LANE 2: PREPARANDO */}
                            <div style={{ display: 'flex', flexDirection: 'column', height: '100%', borderLeft: `1px solid ${Colors.kds.border}`, paddingLeft: Spacing['2xl'] }}>
                                <h2 style={{ fontSize: Typography.uiTiny.fontSize, fontWeight: 'bold', color: Colors.info, marginBottom: Spacing.lg, textTransform: 'uppercase', letterSpacing: '0.2em', display: 'flex', justifyContent: 'space-between' }}>
                                ...
                                <div style={{ flex: 1, overflowY: 'auto', paddingRight: Spacing.sm, paddingBottom: Spacing['4xl'] }}>
                                    ...
                                    {preparingOrders.length === 0 && (
                                        <div style={{ color: Colors.kds.text.dim, textAlign: 'center', paddingTop: Spacing['2xl'], fontStyle: 'italic' }}>Bancada livre.</div>
                                    )}
                                </div>
                            </div>
                        </motion.div>
