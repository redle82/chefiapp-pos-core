    ...
    useEffect(() => {
        ...
        async function fetchVerifiedMenu() {
            try {
                ...
                if (!tenantId) {
                    const stored = getTabIsolated('chefiapp_blueprint') || getTabIsolated('chefiapp_system_blueprint_v2'); // Legacy/Core key
                    if (stored) {
                        try {
                ...
                if (!tenantId) {
                    tenantId = getTabIsolated('chefiapp_active_tenant') || getTabIsolated('chefiapp_restaurant_id') || getTabIsolated('chefiapp_tenant_id') || undefined;
                }
                ...
                if (!tenantId) {
                    ...
                    console.warn("[useRealMenu] No active tenant found in blueprint or storage.");
                    if (mounted) {
                        setProducts([]);
                        setLoading(false);
                    }
                ...
                if (mounted) {
                    ...
                    const mapped: VerifiedProduct[] = safeData.map((item: any) => {
