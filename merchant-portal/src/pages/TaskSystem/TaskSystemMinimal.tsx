...
import { readOpenTasks, readTasksByOrder, readTasksByItem } from '../../core-boundary/readers/TaskReader';
import { dockerCoreClient } from '../../core-boundary/docker-core/connection';
import { deduplicateCoreTasks } from '../../core/tasks/TaskFiltering';
import { useRestaurantIdentity } from '../../core/identity/useRestaurantIdentity';
import { getTabIsolated } from '../../core/storage/TabIsolatedStorage';
import { generateTasks, generateScheduledTasks } from '../../core-boundary/writers/TaskWriter';
...
export function TaskSystemMinimal() {
  ...
  useEffect(() => {
    ...
    console.log('[TaskSystem] Restaurant ID resolved:', id, 'identity.loading:', identity.loading);
    setRestaurantId(id);
      ...
      channel = dockerCoreClient
        ...
        .subscribe((status) => {
          if (status === 'SUBSCRIBED') {
            ...
            console.warn('[TaskSystem] Realtime subscription failed, using polling fallback');
          }
        });
    } catch (err) {
      console.warn('[TaskSystem] Failed to setup Realtime, using polling fallback:', err);
    }
  ...
  }, [finalRestaurantId, stationFilter, statusFilter, loadingIdentity]);
  ...
  const loadTasks = async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      console.log('[TaskSystem] Loading tasks for restaurant:', finalRestaurantId);
      ...
      if (statusFilter === 'OPEN') {
        ...
        console.log('[TaskSystem] Loaded', tasksData.length, 'open tasks');
      } else {
      ...
      const errorMsg = err instanceof Error ? err.message : 'Erro ao carregar tarefas';
  ...
  const handleAcknowledge = async (taskId: string) => {
    try {
      ...
      setError(err instanceof Error ? err.message : 'Erro ao reconhecer tarefa');
    }
  };
  ...
  const getTaskTypeIcon = (type: string) => {
    switch (type) {
      case 'ATRASO_ITEM': return '⏱️';
      case 'ACUMULO_BAR': return '🍺';
      case 'ENTREGA_PENDENTE': return '📦';
      case 'ITEM_CRITICO': return '🚨';
      case 'PEDIDO_ESQUECIDO': return '⚠️';
      case 'ESTOQUE_CRITICO': return '📉';
      case 'RUPTURA_PREVISTA': return '🔴';
      case 'EQUIPAMENTO_CHECK': return '🔧';
      default: return '📋';
    }
  };
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                    ...
                    <span style={{ 
                      ...
                      backgroundColor: `${getStationColor(task.station || undefined)}20`,
                      padding: '2px 8px',
                      borderRadius: '4px',
                    }}>
                      {task.station || 'N/A'}
                  ...
                  {task.context && Object.keys(task.context).length > 0 && (
                    <div style={{ 
                      ...
                      <strong>Contexto:</strong> {JSON.stringify(task.context, null, 2)}
                    </div>
                  )}
