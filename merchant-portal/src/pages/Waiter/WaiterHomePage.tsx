...
// Estado demo: 12 mesas
const DEMO_TABLES: Table[] = [
  { id: '1', number: 1, status: TableStatus.FREE },
  { id: '2', number: 2, status: TableStatus.CALLING, callCount: 3, lastCallAt: new Date(Date.now() - 120000) },
...
export function WaiterHomePage() {
  ...
  const callsCount = useMemo(() => {
    return tables.filter(t => t.status === TableStatus.CALLING || t.status === TableStatus.BILL_REQUESTED).length;
  }, [tables]);
