...
export function InventoryStockMinimal() {
  ...
  const [newIngredientUnit, setNewIngredientUnit] = useState<CoreIngredient['unit']>('unit');
  ...
  const loadAllData = async () => {
    try {
      ...
      const [locs, eq, ing, stock, bom, prods] = await Promise.all([
        ...
        dockerCoreClient.from('gm_products').select('id,name').eq('restaurant_id', restaurantId).order('name', { ascending: true }).then((r) => (r.data || []) as { id: string; name: string }[]).catch(() => []),
      ]);
  ...
  const handleCreateIngredient = async (e: React.FormEvent) => {
    ...
    try {
      await dockerCoreClient.from('gm_ingredients').insert({
  ...
  const handleCreateBOM = async (e: React.FormEvent) => {
    ...
    try {
      await dockerCoreClient.from('gm_product_bom').insert({
        restaurant_id: restaurantId,
  ...
  return (
    <div style={{ padding: '24px', maxWidth: '1400px', margin: '0 auto' }}>
      ...
      {activeTab === 'ingredients' && (
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
            <h2 style={{ fontSize: '20px', fontWeight: 'bold' }}>Ingredientes</h2>
            <button
              type="button"
            ...
            >
              <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                ...
                <input
                  ...
                  required
                  style={{ padding: '8px 12px', fontSize: '14px', minWidth: 160 }}
                />
              </label>
              <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                ...
                <select
                  ...
                  onChange={(e) => setNewIngredientUnit(e.target.value as CoreIngredient['unit'])}
              ...
              <button type="submit" disabled={savingIngredient} style={{ padding: '8px 16px', backgroundColor: '#059669', color: '#fff', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '14px' }}>
          ...
          <div style={{ display: 'grid', gap: '12px' }}>
            {ingredients.length === 0 ? (
              <p style={{ color: '#666', padding: '20px', textAlign: 'center' }}>
                Nenhum ingrediente cadastrado. Clique em &quot;Novo ingrediente&quot; para criar.
              </p>
            ) : (
              ingredients.map((ing) => (
                <div
      ...
      {activeTab === 'recipes' && (
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
            <h2 style={{ fontSize: '20px', fontWeight: 'bold' }}>Receitas (BOM)</h2>
            <button
              type="button"
              ...
              style={{
                padding: '8px 16px',
                backgroundColor: products.length && ingredients.length ? '#3b82f6' : '#9ca3af',
                color: '#fff',
                border: 'none',
                borderRadius: '6px',
                cursor: products.length && ingredients.length ? 'pointer' : 'not-allowed',
                fontSize: '14px',
              }}
            >
            ...
            >
              <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                ...
                <select
                  ...
                  required
                  style={{ padding: '8px 12px', fontSize: '14px', minWidth: 180 }}
                >
              ...
              <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                <span style={{ fontSize: '13px', color: '#666' }}>Ingrediente</span>
                <select
              ...
              <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                <span style={{ fontSize: '13px', color: '#666' }}>Qtd. por unidade</span>
                <input
                  type="text"
                  inputMode="decimal"
                  value={newRecipeQty}
              ...
              <button type="submit" disabled={savingRecipe} style={{ padding: '8px 16px', backgroundColor: '#059669', color: '#fff', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '14px' }}>
          ...
          <div style={{ display: 'grid', gap: '12px' }}>
            {productBOM.length === 0 ? (
              <p style={{ color: '#666', padding: '20px', textAlign: 'center' }}>
                Nenhuma receita cadastrada. Crie produtos e ingredientes primeiro, depois &quot;Nova receita&quot;.
              </p>
            ) : (
              productBOM.map((bom: any) => {
                const productName = products.find((p) => p.id === bom.product_id)?.name || bom.product_id;
                const ingredientName = bom.ingredient?.name || bom.ingredient_id;
