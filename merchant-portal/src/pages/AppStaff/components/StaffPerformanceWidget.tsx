...
export const StaffPerformanceWidget: React.FC<StaffPerformanceWidgetProps> = ({
    ...
    useEffect(() => {
        ...
        const fetchPerformance = async () => {
            ...
            (shifts || []).forEach((shift: any) => {
                ...
                const shiftActions = (actions || []).filter((a: any) => a.shift_id === shift.id);
            ...
            const result = Array.from(employeeMap.values()).map(emp => ({
                ...emp,
                avgActionsPerHour: emp.totalMinutes > 0
    ...
    const getPerformanceBadge = (avgActions: number) => {
        if (avgActions >= 5) return { status: 'ready' as const, label: '⭐ Excelente' };
        if (avgActions >= 3) return { status: 'warning' as const, label: 'Bom' };
        if (avgActions >= 1) return { status: 'warning' as const, label: 'Regular' };
    ...
    if (loading) {
        return (
            <Card surface="layer1" padding="lg">
                <Text size="sm" color="tertiary">A carregar performance...</Text>
            </Card>
        );
    }
    ...
    return (
        <Card surface="layer1" padding="lg">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                <Text size="lg" weight="bold" color="primary">👥 Performance da Equipa</Text>
                    ...
                    {performance.map((emp, idx) => {
                        ...
                        return (
                            <div key={emp.employeeId} style={{
                                ...
                                alignItems: 'center',
                                padding: 12,
                                backgroundColor: idx === 0 ? `${colors.success.base}15` : colors.surface.layer2,
                                borderRadius: 8,
                                border: idx === 0 ? `1px solid ${colors.success.base}40` : 'none'
                            }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                    <span style={{ fontSize: 24 }}>{idx === 0 ? '🏆' : getRoleEmoji(emp.role)}</span>
                                    <div>
                                        <Text size="md" weight="bold" color="primary">{emp.employeeName}</Text>
                                        <Text size="xs" color="secondary">
                                            {emp.shiftsCount} turnos • {Math.round(emp.totalMinutes / 60)}h trabalhadas
                                        </Text>
                                    </div>
                                </div>
                                <div style={{ textAlign: 'right' }}>
                                    <Text size="lg" weight="black" color="action">{emp.avgActionsPerHour}</Text>
                                    <Text size="xs" color="tertiary">ações/hora</Text>
