...
export function PortioningTaskView({ task, onComplete }: PortioningTaskViewProps) {
  ...
  const baseProductId = task.context?.base_product_id;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(`${CONFIG.API_BASE}/api/portioning/measurements`, {
        method: 'POST',
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 24, padding: 16 }}>
        ...
        <Card surface="layer2" padding="lg">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            <div>
              <Text size="xs" color="secondary">Gramatura</Text>
              <Text size="2xl" weight="bold">{targetWeight}g</Text>
            </div>
            <div>
              <Text size="xs" color="secondary">Espessura</Text>
              <Text size="2xl" weight="bold">{targetThickness}mm</Text>
            </div>
            <div style={{ gridColumn: '1 / -1' }}>
              <Text size="xs" color="secondary">Porções</Text>
              <Text size="xl" weight="bold">{targetPortions} porções</Text>
            </div>
          </div>
        </Card>
        ...
        {/* Variation Buttons */}
        <div>
          <Text size="md" weight="bold" color="primary" style={{ marginBottom: 16 }}>
            Medição Real
          </Text>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 12 }}>
            {variations.map((variation) => (
              <Button
                ...
                variant={selectedVariation === variation.value ? 'primary' : 'outline'}
                size="lg"
                style={{
                  minHeight: 80,
                  borderColor: selectedVariation === variation.value ? variation.color : undefined,
                  borderWidth: selectedVariation === variation.value ? 3 : 1,
                }}
              >
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                  <Text size="sm" weight="bold">{variation.label}</Text>
                  {variation.value !== 0 && (
                    <Text size="xs" color="secondary">
                      {variation.value > 0 ? '+' : ''}{variation.value}g
                    </Text>
                  )}
                </div>
              </Button>
