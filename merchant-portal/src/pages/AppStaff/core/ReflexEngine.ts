  ...
  useEffect(() => {
    ...
    const processReflexes = async () => {
      ...
      for (const order of paidOrders) {
        ...
        // gm_tasks: criar tarefa via RPC (fonte canónica)
        const { createTaskRpc } = await import(
          "../../../infra/writers/TaskWriter"
        );
        const message = `Limpar Mesa ${order.tableNumber}: Cliente pagou. Mesa livre.`;
        try {
          const coreTaskId = await createTaskRpc({
            restaurantId: rId,
          ...
          setTasks((prev) => {
            if (prev.some((t) => t.id === coreTaskId)) return prev;
            return [...prev, newTask];
          });
          notifyActivity();
        } catch (err) {
          console.error("Reflex createTaskRpc failed:", err);
        }
      }
    };
