...
export const ManagerDashboard: React.FC = () => {
    ...
    // Risk Evaluation
    const getHealthStatus = (risk: number): { status: 'ready' | 'warning' | 'error'; label: string; icon: string } => {
        if (risk < 30) return { status: 'ready', label: 'Healthy Flow', icon: '🟢' };
        if (risk < 70) return { status: 'warning', label: 'High Tension', icon: '🟡' };
    ...
    return (
        ...
        >
            <div style={{ maxWidth: 600, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 24, width: '100%' }}>
                ...
                {/* 0. PILOT MODE BANNER */}
                <div style={{
                    background: `linear-gradient(90deg, ${colors.action.base}20, ${colors.action.base}10)`,
                    border: `1px solid ${colors.action.base}40`,
                    borderRadius: 8,
                    padding: '12px 16px',
                    display: 'flex',
                    alignItems: 'center',
                    ...
                    <div>
                        <Text size="sm" weight="bold" color="action">MODO PILOTO</Text>
                ...
                <Card
                    ...
                    style={{
                        ...
                        textAlign: 'center',
                        gap: 16,
                        border: isAlive ? `1px solid ${colors.success.base}` : `1px solid ${colors.border.subtle}`,
                        transition: 'all 0.5s ease'
                    }}
                >
                    <div style={{ fontSize: 48, filter: isAlive ? 'drop-shadow(0 0 10px rgba(0,255,0,0.5))' : 'none' }}>
                        {health.icon}
                    </div>
                    <div>
                        <Text size="2xl" weight="black" color="primary">{health.label}</Text>
                        <div style={{ marginTop: 8 }}>
                            <Badge status={health.status} label={`RISK: ${currentRiskLevel.toFixed(1)}%`} />
                        </div>
                    </div>
                </Card>
                ...
                {hungerSignals.length > 0 && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                        <Text size="xs" weight="bold" color="destructive">METABOLIC HUNGER ({hungerSignals.length})</Text>
                        {hungerSignals.map(signal => (
                            <Card key={signal.itemId} surface="layer2" padding="md" style={{ borderLeft: `4px solid ${colors.destructive.base}` }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <div>
                                        <Text size="lg" weight="bold" color="primary">{signal.itemId}</Text>
                                        <Text size="sm" color="destructive">Stock Critical</Text>
                                    </div>
                                    <Button size="sm" tone="destructive">Ack</Button>
                                </div>
                            </Card>
                        ))}
                    </div>
                ...
                {alerts.length > 0 && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                        <Text size="xs" weight="bold" color="action">METABOLIC INTELLIGENCE</Text>
                        {alerts.map(alert => (
                            <Card key={alert.itemId} surface="layer2" padding="md">
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <Text size="md" weight="bold" color="primary">{alert.itemId}</Text>
                                    <Badge status="warning" label="PREDICTION" size="sm" />
                                </div>
                                <div style={{ marginTop: 8, display: 'flex', justifyContent: 'space-between' }}>
                                    <Text size="sm" color="tertiary">Panic Score: {alert.panicScore.toFixed(1)}</Text>
                                    {alert.daysUntilStockout < 0.5 && (
                                        <Text size="sm" color="destructive" weight="bold">Stockout &lt; 12h</Text>
                                    )}
                                </div>
                            </Card>
                        ))}
                ...
                {/* 4. INTERVENTIONS */}
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    <Text size="xs" weight="bold" color="secondary">ACTIVE INTERVENTIONS ({interventionTasks.length})</Text>
                    {interventionTasks.length === 0 ? (
                        <div style={{ padding: 24, textAlign: 'center', border: `1px dashed ${colors.border.subtle}`, borderRadius: 8 }}>
                            <Text size="sm" color="tertiary">No manual interventions required.</Text>
                        </div>
                    ) : (
                        interventionTasks.map(task => (
                            <Card key={task.id} surface="layer2" padding="md">
                                <Text size="md" weight="bold" color="primary">{task.title}</Text>
                                <Text size="sm" color="secondary">{task.assigneeRole} • {task.status}</Text>
                            </Card>
                        ))
                    )}
                </div>
                    ...
                    {integrationsReady ? (
                        integrations.map(integration => (
                            <Card key={integration.id} surface="layer2" padding="md" style={{
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <div>
                                        <Text size="md" weight="bold" color="primary">{integration.name}</Text>
                        ...
                        <div style={{ padding: 24, textAlign: 'center', border: `1px dashed ${colors.border.subtle}`, borderRadius: 8 }}>
                            <Text size="sm" color="tertiary">Carregando integrações...</Text>
                        </div>
                    )}
                </div>
                        ...
                        {showDevPanel && (
                            <Card surface="layer2" padding="lg" style={{
                                ...
                                <div style={{
                                    ...
                                    <div>
                                        <Text size="xs" color="tertiary">Events</Text>
                                        <Text size="lg" weight="bold" color="primary">{mockStats.eventCount}</Text>
                                    </div>
                                    <div>
                                        <Text size="xs" color="tertiary">Orders</Text>
                                        <Text size="lg" weight="bold" color="primary">{mockStats.ordersInFlight}</Text>
                                    </div>
                                    <div>
