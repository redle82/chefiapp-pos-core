...
import { useOnboardingState, useGhostPreviewProps } from '../hooks'
...
export function SetupLayout() {
  ...
  function getStepIcon(status: 'completed' | 'current' | 'blocked' | 'pending') {
    switch (status) {
  ...
  const progressPct = Math.max(0, Math.min(100, Math.round((completedCount / 5) * 100)))

  const slug = hasIdentity ? (profile?.slug || getTabIsolated('chefiapp_slug') || '') : ''
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => controller.abort(), 1200)
      ; (async () => {
        try {
          const res = await fetch(`${apiBase.replace(/\/$/, '')}/health`, { signal: controller.signal })
          if (cancelled) return
  ...
  return (
    <OnboardingContext.Provider value={onboarding}>
      <OSFrame context="onboarding">
        <div style={{ color: '#f5f5f7', display: 'flex', flexDirection: 'column', flex: 1 }}>
          <header style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div style={{ fontSize: 13, fontWeight: 600, opacity: 0.6, paddingLeft: 0 }}>
            ...
            <div style={{
              ...
              <aside style={{ display: 'grid', gap: 12 }}>
                <div className="card" style={{ margin: 0 }}>
                  {!hasIdentity ? (
                    <>
                      <div className="h3" style={{ marginBottom: 6 }}>Vamos começar</div>
                      <div className="muted">
                        Cria a identidade do teu restaurante e vê a tua página ganhar vida.
                      </div>
                      <div className="banner neutral" style={{ marginTop: 12, padding: 10 }}>
                        <div className="bannerText">A pré-visualização aparecerá aqui após criares a identidade.</div>
                      </div>
                    </>
                  ) : (
                    <>
                      <div className="h3" style={{ marginBottom: 6 }}>A construir</div>
                      <div style={{ fontWeight: 800, fontSize: 14, letterSpacing: '-0.2px' }}>{restaurantName}</div>
                      ...
                      {!steps.published && (
                        <div className="banner neutral" style={{ marginTop: 10, padding: 10 }}>
                          <div className="bannerText">A pré-visualização da tua página aparecerá aqui quando publicares.</div>
                        </div>
                      )}
                      ...
                      {steps.published && (
                        <div style={{ marginTop: 10 }}>
                          {health === 'ok' ? (
                            <div className="banner ok" style={{ marginTop: 0, padding: 10 }}>
                              <div className="bannerText">✓ Servidor ligado • preview disponível</div>
                            </div>
                          ) : health === 'offline' ? (
                            <div className="banner neutral" style={{ marginTop: 0, padding: 10 }}>
                              <div className="bannerText">Liga o servidor para veres a pré-visualização em tempo real.</div>
                            </div>
                          ) : (
                            <div className="banner neutral" style={{ marginTop: 0, padding: 10 }}>
                              <div className="bannerText">A verificar ligação ao servidor…</div>
                            </div>
                          )}
                        </div>
                      )}

                      {hasIdentity && <GhostPreviewWrapper steps={steps} health={health} publicUrl={publicUrl} />}
                    </>
                  )}
                </div>
                ...
                >
                  {navItems.map(({ to, label, step }) => {
                    ...
                    return (
                      ...
                      >
                        <span style={{ width: 18, textAlign: 'center' }}>{getStepIcon(status)}</span>
                        {label}
                      </NavLink>
                    )
                  })}
                ...
                {/* Gate banner */}
                {gates.ok === false && (
                  <div className="banner warn" style={{ marginTop: 0, padding: 12 }}>
                    <div className="bannerTitle">💳 Assinatura Pendente</div>
                    <div className="bannerText">
                      {gates.message || 'Ativa o ChefIApp Pro para publicares a tua página.'}{' '}
                      <NavLink to="/app/setup/billing" style={{ textDecoration: 'underline', fontWeight: 700 }}>
                        Ativar agora
                      </NavLink>
                    </div>
                  </div>
                ...
                {gates.ok === true && (
                  <div className="banner ok" style={{ marginTop: 0, padding: 12 }}>
                    <div className="bannerTitle">✓ Plano Ativo</div>
                    <div className="bannerText">ChefIApp Pro • {profile?.web_level || 'FULL'}</div>
                  </div>
                )}
