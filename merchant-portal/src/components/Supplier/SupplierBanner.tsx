...
export const SupplierBanner: React.FC<SupplierBannerProps> = ({ placement }) => {
    if (!placement || !placement.campaign || !placement.campaign.assets.banner_mobile) {
        return null;
    }
      ...
      */}
            {campaign.assets.banner_mobile.startsWith('http') ? (
                <img
                    ...
                    alt={campaign.name}
                    style={{ width: '100%', display: 'block', objectFit: 'cover', maxHeight: '120px' }}
                />
            ) : (
                <div style={{ padding: '1rem', textAlign: 'center', backgroundColor: '#333', color: '#fff' }}>
                    <small style={{ opacity: 0.7, textTransform: 'uppercase', fontSize: '10px' }}>Parceiro Oficial</small>
