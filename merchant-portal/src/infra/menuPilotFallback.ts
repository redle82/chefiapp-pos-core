...
export function getPilotProducts(restaurantId: string): PilotProductStored[] {
  try {
    const raw = typeof localStorage !== "undefined" ? localStorage.getItem(storageKey(restaurantId)) : null;
    if (!raw) return [];
...
export function addPilotProduct(
  ...
  const next = exists ? list.map((p) => (p.id === product.id ? { ...p, ...product } : p)) : [...list, { ...product, restaurant_id: restaurantId }];
  try {
    if (typeof localStorage !== "undefined") localStorage.setItem(storageKey(restaurantId), JSON.stringify(next));
  } catch (_) {}
}
...
export function isBackendUnavailable(err: unknown): string | false {
  ...
  if (typeof err === "object" && err !== null && "message" in err) {
    ...
    if (/network|failed to fetch|load failed|ECONNREFUSED|ETIMEDOUT|502|503|504/i.test(msg)) return msg || "Ligação indisponível";
  }
...
export function isNetworkError(err: unknown): boolean {
  ...
  if (typeof err === "object" && err !== null && "code" in err) {
    ...
    if (["ECONNREFUSED", "ETIMEDOUT", "ENOTFOUND", "ERR_NETWORK"].includes(code)) return true;
  }
  const msg = err instanceof Error ? err.message : String(err);
  return /network|failed to fetch|load failed|timeout|ECONNREFUSED|ETIMEDOUT/i.test(msg);
}
