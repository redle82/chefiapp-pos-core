...
// ─────────────────────────────────────────────────────────────
// SAMPLE DATA (realistic para demos)
// ─────────────────────────────────────────────────────────────

const SAMPLE_ITEMS: MockOrderItem[] = [
  { sku: 'burger-classic', name: 'Classic Burger', qty: 1, unitPrice: 18.90 },
  { sku: 'fries-large', name: 'Batata Grande', qty: 1, unitPrice: 12.00 },
  { sku: 'coke-zero', name: 'Coca Zero 350ml', qty: 2, unitPrice: 6.00 },
  { sku: 'milkshake-choc', name: 'Milkshake Chocolate', qty: 1, unitPrice: 15.00 },
  { sku: 'onion-rings', name: 'Onion Rings', qty: 1, unitPrice: 14.00 },
];

const SAMPLE_CUSTOMERS = [
  { name: 'João Silva', phone: '11999001234', address: 'Rua Augusta 1500, Consolação' },
  { name: 'Maria Santos', phone: '11988765432', address: 'Av. Paulista 1000, Bela Vista' },
  { name: 'Pedro Oliveira', phone: '11977654321', address: 'Rua Oscar Freire 800, Jardins' },
  { name: 'Ana Costa', phone: '11966543210', address: 'Rua Haddock Lobo 400, Cerqueira César' },
];
...
export class MockDeliveryAdapter implements IntegrationAdapter {
  ...
  async onEvent(event: IntegrationEvent): Promise<void> {
    ...
    if (event.type === 'order.completed') {
      ...
      if (order) {
        ...
        console.log(`[MockDelivery] 📦 Order ${order.orderId} marked as delivered`);
      }
    }
  }
  ...
  simulateNewOrder(): MockOrder {
    ...
    console.log(`[MockDelivery] 📥 New order: ${order.orderId} - R$${order.total.toFixed(2)}`);
    return order;
  }
