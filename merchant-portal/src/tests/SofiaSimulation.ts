...
import { SOFIA_RITUALS } from '../core/inventory/SofiaModel';
...
const runSimulation = () => {
    ...
    const tuesdayDraft = generatePurchaseDraft('staff-jose', [], [
        { item: SOFIA_INVENTORY.find(i => i.id === 'keg-super-bock')!, qty: 2, reason: 'impulse_panic' }
    ]);
    ...
    console.log(`- Panic Value: €${(tuesdayDraft.panicWastePotentialEur / 100).toFixed(2)}\n`);
    ...
    const wednesdayDraft = generatePurchaseDraft('staff-maria', wednesdaySignals, []);
    ...
    console.log(`- Status: ${wednesdayDraft.requiresWitness ? 'TRAVADO' : 'LIBERADO (Verde)'}\n`);

    // ---------------------------------------------------
    // SCENARIO 3: PIZZA (Biologia vs Medo)
    // ---------------------------------------------------
    console.log("🍕 CENÁRIO 3: PIZZA (O Freio Metabólico)");
    const pizzaItem = SOFIA_INVENTORY.find(i => i.id === 'base-pizza-artesanal')!;

    // Sub-case A: Necesidade Real (Stock 15, Critical 20)
    console.log(`- Estoque Atual: ${pizzaItem.currentStock} (Crítico: ${pizzaItem.lifecycle.criticalLevel})`);

    // Sub-case B: Tentativa de Excesso (Buy +50, MaxSafe 60)
    // Current 15 + Buy 50 = 65. Max Safe is 60.
    const hoardingDraft = generatePurchaseDraft('staff-jose', [], [
        { item: pizzaItem, qty: 50, reason: 'event_prep' } // Intent: "Event"
    ]);

    console.log(`- Tentativa: Comprar 50 pizzas (Justificativa: 'Evento')`);
    console.log(`- Estoque Projetado: ${15 + 50} (Teto Seguro: ${pizzaItem.lifecycle.maxSafeStock})`);
    console.log(`- Veredito do Sistema: ${hoardingDraft.items[0].reason}`); // Should be overridden to impulse_panic?
    console.log(`- Aviso: "Estoque acima do teto de segurança. Risco de validade."\n`);
    ...
    missions.forEach(m => {
        console.log(`  > [${m.role.toUpperCase()}] Contar ${m.itemName} (${m.reason})`);
    });
    console.log("");
