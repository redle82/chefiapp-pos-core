...
try {
    ...
    console.warn('PrinterService: react-native-tcp-socket not found (Native Module missing). Printing will be disabled.');
    TcpSocket = {
        createConnection: () => ({
            on: () => { },
            write: () => { },
...
export class PrinterService {
    ...
    async getConfig(type: 'KITCHEN' | 'COUNTER') {
        try {
            const portStr = await AsyncStorage.getItem('@chefiapp_printer_port') || '9100';
            const port = parseInt(portStr, 10);

            let host = '';
            if (type === 'KITCHEN') {
                host = await AsyncStorage.getItem('@chefiapp_kitchen_ip') || '192.168.1.200';
            } else {
                host = await AsyncStorage.getItem('@chefiapp_counter_ip') || '192.168.1.201';
            }
            return { host, port };
        } catch (e) {
    ...
    async printTicket(table: string, items: any[]): Promise<boolean> {
        try {
            const connected = await this.connect('KITCHEN');
            if (!connected) throw new Error("Falha ao conectar com a impressora (Cozinha). Verifique o IP.");
            ...
            const commands = encoder
                ...
                .align('center')
                .bold(true)
                .line(`TICKET #${Math.floor(Math.random() * 1000)}`) // Mock Ticket ID
                .line(`MESA ${table}`)
                .bold(false)
    ...
    async printReceipt(order: any): Promise<boolean> {
        try {
            ...
            const commands = encoder
                ...
                .align('center')
                .bold(true)
                .line('GOLDMONKEY RESTAURANT') // TODO: Get from Settings/Context
            ...
            commands
                ...
                .align('right')
                .bold(true)
                .line(`TOTAL: €${(order.total_amount / 100).toFixed(2)}`) // Assuming total_amount is in cents
                .bold(false)
                .align('center')
                .newline()
