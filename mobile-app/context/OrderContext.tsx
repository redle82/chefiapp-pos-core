...
import { emitAchievementUnlocked } from "@/utils/achievementEvents";
...
export interface OrderContextType {
  ...
  splitOrder: (
    ...
    itemIds: string[],
  ) => Promise<string | null>; // New: returns new Order ID
...
export const OrderProvider = ({ children }: { children: ReactNode }) => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      if (isNetworkFailure || isClientMismatch) {
        ...
        if (__DEV__)
          console.warn(
            "[OrderContext] Core unreachable or client mismatch, orders empty:",
            msg,
          );
      } else {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        async (payload) => {
          console.log(
            "⚡️ [REALTIME] gm_orders Update Received:",
            payload.eventType,
          );
          ...
          if (payload.eventType === "INSERT") {
            ...
            if (newOrder.origin === "WEB_PUBLIC" || newOrder.origin === "web") {
              ...
              console.log(
                "[Push] Notificação enviada para novo pedido web:",
                newOrder.id,
              );
            }
          }
  ...
  const submitOrder = async () => {
    ...
    if (shiftState !== "active" || !shiftId) {
      Alert.alert(
        "Turno Fechado",
        "Você precisa iniciar um turno para enviar pedidos.",
      );
      return;
    }
    ...
    try {
      ...
      let orderId = orders.find(
        (o) => o.table === activeTableId && o.status !== "paid",
      )?.id;
      ...
      try {
        ...
        Alert.alert(
          "Erro na Impressora",
          pe.message || "Falha ao imprimir, mas o pedido foi salvo.",
        );
      }
    } catch (e) {
      const error = e instanceof Error ? e : new Error(String(e));
      ...
      const offlineOrderId = activeTableId
        ? orders.find((o) => o.table === activeTableId && o.status !== "paid")
            ?.id || generateUUID()
        : generateUUID();
      ...
      Alert.alert(
        "Sem Conexão",
        "Pedido salvo offline. Será enviado quando a internet voltar.",
      );
      clearDraft();
      ...
      try {
        ...
        console.error("Kitchen Print Error", pe);
        Alert.alert(
          "Erro na Impressora (Offline)",
          pe.message || "Pedido salvo, mas falha ao imprimir.",
        );
      }
    }
  };
  ...
  const quickPay = async (
    ...
    try {
      ...
      if (!validation.canPay) {
        Alert.alert(
          "Pagamento Bloqueado",
          validation.reason || "Pedido não pode ser pago.",
        );
        return false;
      }
        ...
        if (error.message.includes("payment_method")) {
          ...
          if (retryError) {
            ...
            if (currentOrder?.status === "paid") {
              console.log(
                `[FastPay] Order ${orderId} already paid (idempotent check)`,
              );
              return true;
            }
            throw retryError;
      ...
      if (!updatedOrder) {
        console.log(
          `[FastPay] Order ${orderId} already paid (no rows updated)`,
        );
        return true;
      }
      ...
      try {
          ...
          gamificationService
            ...
            .then((newAchievements) => {
              if (newAchievements.length > 0) {
                console.log(
                  "[OrderContext] New achievements unlocked:",
                  newAchievements,
                );
                newAchievements.forEach(emitAchievementUnlocked);
              }
            })
            .catch((err) => {
        ...
        console.warn(
          "[OrderContext] Failed to award gamification points:",
          gamificationError,
        );
    ...
    try {
      ...
      if (dbStatus === "paid") {
          ...
          if (user?.id && order) {
            ...
            if (restaurantId) {
              ...
              gamificationService
                ...
                .catch((err) => {
                  console.warn(
                    "[OrderContext] Failed to check achievements:",
                    err,
                  );
                });
            }
        ...
        if (order) {
          ...
          try {
            const autoPrint = await AsyncStorage.getItem(
              "@chefiapp_auto_print",
            );
            if (autoPrint !== "false") {
              // Default true
