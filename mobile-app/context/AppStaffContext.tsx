...
const MOCK_TASKS: Task[] = [
  ...
  {
    id: "t1",
    title: "Verificar mesa 5",
    priority: "attention",
    status: "pending",
    assignedRoles: ["waiter", "manager"],
    category: "table_service",
    createdAt: Date.now(),
  },
  {
    id: "t2",
    title: "Levar pedido mesa 3",
    priority: "urgent",
    status: "pending",
    assignedRoles: ["waiter"],
    category: "delivery",
    createdAt: Date.now(),
  },
  {
  ...
  {
    id: "t4",
    title: "Preparar Risotto especial",
    priority: "critical",
    status: "pending",
    assignedRoles: ["cook", "chef"],
    category: "kitchen",
    createdAt: Date.now(),
  },
  {
    id: "t5",
    title: "Mise en place - legumes",
    priority: "attention",
    status: "pending",
    assignedRoles: ["cook"],
    category: "prep",
  ...
  {
    id: "t6",
    title: "Verificar temperatura frigorífico",
    priority: "background",
    status: "pending",
    assignedRoles: ["cook", "chef"],
    category: "quality",
  ...
  {
    id: "t9",
    title: "Limpar WC - rotina 14h",
    priority: "attention",
    status: "pending",
    assignedRoles: ["cleaning"],
    category: "cleaning",
    createdAt: Date.now(),
  },
  {
  ...
  {
    id: "t11",
    title: "Resolver reclamação mesa 8",
    priority: "critical",
    status: "pending",
    assignedRoles: ["manager", "owner"],
    category: "customer",
    createdAt: Date.now(),
  },
  {
    id: "t12",
    title: "Ajustar horários próxima semana",
    priority: "background",
    status: "pending",
    assignedRoles: ["manager", "owner"],
  ...
  {
    id: "t13",
    title: "Preparar carrinho",
    priority: "attention",
    status: "pending",
    assignedRoles: ["ambulante"],
    category: "basic",
    createdAt: Date.now(),
  },
  {
...
const AppStaffContext = createContext<AppStaffContextType | undefined>(
  undefined,
);
...
export function AppStaffProvider({ children }: { children: React.ReactNode }) {
  ...
  // Operational context
  const [operationalContext, setOperationalContextState] =
    useState<OperationalContext>({
  ...
  const [financialState, setFinancialState] =
    useState<FinancialState>("drawer_closed");
  const [financialSessionId, setFinancialSessionId] = useState<string | null>(
    null,
  );
  ...
  const roleConfig = useMemo(
    () => ROLE_PERMISSIONS_MAP[activeRole],
    [activeRole],
  );
  ...
  const fetchTasks = useCallback(
    async (overrideRestaurantId?: string) => {
      try {
        const restaurantId =
          overrideRestaurantId ?? operationalContext.businessId;
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (data) {
          ...
          if (data.financialSessionId)
            setFinancialSessionId(data.financialSessionId);
          if (data.operationalContext)
            setOperationalContextState(data.operationalContext);
        }

        // 2. Load Cloud Identity (Restaurant) - CRÍTICO
        let resolvedBusinessId = operationalContext.businessId;
        let resolvedBusinessName = operationalContext.businessName;
        ...
        try {
          ...
          if (restData) {
            resolvedBusinessId = restData.id;
            resolvedBusinessName = restData.name;
          } else {
        ...
        // 3. Load Cloud Tasks
        await fetchTasks(resolvedBusinessId);
        ...
        try {
          ...
          if (catData) {
            setCategoryLinks(
              catData.map((c) => ({ id: c.id, station_id: c.station_id })),
            );
          } else {
        ...
        const isValidBusinessId =
          resolvedBusinessId &&
          resolvedBusinessId !== "fallback-restaurant-id" &&
          resolvedBusinessId.length > 10; // UUID ou ID válido
        ...
        if (isValidBusinessId) {
          ...
          setAppState("error");
          setErrorMessage(
            "Não foi possível carregar dados do restaurante. Tente novamente.",
          );
        }
        setIsLoaded(true);
      } catch (error) {
    ...
    const channel = supabase
      .channel("public:gm_tasks")
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "gm_tasks" },
        () => {
          fetchTasks();
        },
      )
  ...
  const setOperationalContext = useCallback(
    (partial: Partial<OperationalContext>) => {
      if (shiftState === "active") {
        console.warn(
          "[AppStaff] Cannot change operational context during active shift",
        );
        return;
      }
  ...
  const startShift = useCallback(async () => {
    ...
    try {
      ...
      const validRestaurantId = /^[0-9a-fA-F-]{36}$/.test(restaurantId)
        ? restaurantId
        : null;
      ...
      if (__DEV__) {
        ...
        setShiftStart(Date.now());
        console.log(
          `[AppStaff] DEV FALLBACK: Using fake shift ID: ${fakeShiftId}`,
        );
      }
    }
  }, [operationalContext.businessId]);
  ...
  const openFinancialSession = useCallback(
  ...
  const closeFinancialSession = useCallback(
  ...
  const endShift = useCallback(
    async (totalRevenue: number, closingCash: number = 0) => {
      // Bug #12 Fix: Validação de permissão SEMPRE antes de executar
      if (!canAccess("shift:end")) {
        Alert.alert(
          "Sem Permissão",
          "Você não tem permissão para encerrar o turno.",
        );
        return;
      }
  ...
  const recordQualityEvent = useCallback(
    async (type: string, scoreImpact: number, metadata: any = {}) => {
      if (!shiftId) {
      ...
      try {
        ...
        const { error } = await supabase.from("gm_quality_events").insert({
  ...
  const createTask = useCallback(
    async (
      title: string,
      role: StaffRole,
      priority: Task["priority"] = "attention",
    ) => {
      try {
        const businessId = operationalContext.businessId;
        // robust check for UUID (approximate)
        const restaurantId = /^[0-9a-fA-F-]{36}$/.test(businessId)
          ? businessId
          : null;
  ...
  const canAccess = useCallback(
    (permission: Permission | string): boolean => {
      ...
      const allowedPermissions = getContextPermissions(
        activeRole,
        operationalContext.maturity,
      );
  ...
  // KDS Helper
  const getStationForCategory = useCallback(
    (categoryId: string): Station | null => {
      // 1. Find link
